...
For same-chain orders, solvers can fill any portion of any leg's output. The proportional share of that leg's escrowed input is released immediately (`inputs[i].amount × fillAmount / totalRequired`). The order is fully filled once every leg reaches its target amount.
...
When a solver provides more tokens than required, the excess is split according to `surplusShareBps` (100% to protocol if the order has calldata). Surplus only applies on the first fill of each output token pair.
