...
Rate bids use `fillOrderAtRate(order, options, inputs)` inside the same ERC-7821 batch. Before signing, the SDK reads `supportsRateFills()` on both the destination gateway and the solver account; missing support refuses the bid. The signed input amounts are maxima, and `previewRateFill` reports their integer-rounded releases. Ordinary bids retain the existing selectors.
