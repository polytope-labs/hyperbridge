...
Bounds per-leg loss if the filler's internal pricing is wrong (bug, stale cache, manipulated venue). For every order, the filler clamps its computed output to at most `(1 + maxOverfillBps) × user-requested amount`. After `maxConsecutiveClamps` consecutive orders where the clamp activated, the strategy halts itself — a pattern that strongly suggests a systemic pricing error — and requires an operator restart. The FX strategy additionally rejects any order where the total output USD value ≥ total input USD value.

Defaults are sensible (`maxOverfillBps = 100` ≈ 1% ceiling, `maxConsecutiveClamps = 3`). Override under `[simplex.overfillProtection]`:

```toml lineNumbers
[simplex.overfillProtection]
maxOverfillBps       = 100   # 1% ceiling above user-requested output
maxConsecutiveClamps = 3     # halt threshold
```
...
When two or more URLs are configured, the event monitor fans every `eth_getLogs` batch out to all providers in parallel. The batch only succeeds when **every** provider returns byte-identical logs (after sorting by `(blockNumber, logIndex, transactionHash)` and normalising BigInt serialisation). Any provider error, timeout, or divergence fails the batch — the scan cursor does not advance, so the same range is retried on the next tick. `getBlockNumber` is likewise fanned out and resolves to the lowest head across providers, preventing a fast node from pulling the scanner ahead of state the slower nodes have not yet indexed.

A single-entry `rpcUrls` array is legal and keeps the pre-existing single-provider behaviour; the quorum machinery only engages when you list more than one endpoint.
...
# Optional — defaults: maxOverfillBps = 100, maxConsecutiveClamps = 3
# [simplex.overfillProtection]
# maxOverfillBps       = 100
