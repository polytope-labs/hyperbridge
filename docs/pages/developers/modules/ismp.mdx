...
This component definesd the state machine identifier of the supported coprocessor, if no coprocessor is supported then this can be `None`.  
Hyperbridge is currently the only coprocessor that supports ISMP.  
* `ConsensusClients` 
