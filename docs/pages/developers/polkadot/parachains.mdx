...
## Parachain State Access with ISMP

This module empowers parachains to interact with specific sibling parachains. Here's how it works:

`Registration`: A parachain can register the IDs of sibling parachains whose state information it requires to access.
`Inherent Mechanism`: The functionality relies on an inherent provider and inherent extrinsics:
    * `Inherent Provider`: This component continuously monitors the ismp-parachain pallet for a list of currently supported parachain IDs.
    * `State Proof Fetching`: The provider then retrieves the latest headers from the relay chain for each supported parachain. Additionally, it fetches state proofs for these headers, which act as cryptographic evidence of the state's validity.
...
`Eliminating Off-chain Consensus Relayer`: By utilizing inherent messages for receiving consensus messages about finalized parachain block headers, the need for a separate off-chain consensus relayer is eliminated.
    `Benefits`:
        ...
        *   `Improved Efficiency`: Inherent messages are a built-in mechanism within the polkadot-sdk, allowing the collator to act as the consensus relayer.
