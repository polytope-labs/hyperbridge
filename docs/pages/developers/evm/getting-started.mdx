...
The Handler contract, which implements `IHandler` interface, is a stateless contract responsible for handling consensus and state proof verifications for all ISMP messages. Upon successful verification, it delegates state persistence and dispatch to the EvmHost contract.
...
Get requests to allow contracts to perform asynchronous reads of a counterparty blockchain's state. When dispatching get requests,
