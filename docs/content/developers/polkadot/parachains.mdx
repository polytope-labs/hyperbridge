...
import addParachain from "./images/add_parachain.png";
...
|------|--------|-------------|
| `add_parachain` | RootOrigin | This call allows a priviledged origin to add a new parachain to the list of connected parachains. When a new parachain is added, consensus relayers will submit state proofs of the parachain's latest header in subsequent consensus messages. The arguments are: 1. **id**: The parachain id of the sibling parachain. 2. **slotDuration**: The slot duration in **milliseconds** for the sibling parachain. |
| `remove_parachain` | RootOrigin | This priviledged call removes a parachain from the list of supported parachains, thereby preventing any future state updates from such parachain. |
