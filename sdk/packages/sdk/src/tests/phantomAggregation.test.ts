...
describe("aggregatePhantomBids bid verification", () => {
	...
	describe("declared Uniswap V4 positions", () => {
		...
		// A bid is the only place a position is named, so an indexer that wants to re-value one
		// after a fill has to learn the tokenId here — the run already verified it, and nothing
		// downstream can rediscover it.
		it("reports the verified position's tokenId so it can be persisted", async () => {
			const userOp = await signedBidUserOp({
				signingKey: SOLVER_KEY,
				paymasterAndData: encodePhantomBidDeclaration({ uniswapV4Positions: [TOKEN_ID] }),
			})
		...
		// Reported positions carry the same ownership guarantee the leg weights do; persisting an
