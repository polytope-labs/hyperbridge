...
pub async fn monitor_clients(
	...
	let lambda = || async {
		for (id, max_interval) in configs.clone() {
			...
			log::trace!(target: crate::LOG_TARGET, "Checking update interval for {:?} on {:?}", id.state_id, hyperbridge_provider.state_machine_id().state_id);
			...
			if current_timestamp
				...
				max_interval
			{
				log::trace!(target: crate::LOG_TARGET, "{:?} -> {:?} Has stalled shutting down", id.state_id, hyperbridge_provider.state_machine_id().state_id);
			...
			log::trace!(target: crate::LOG_TARGET, "Checking update interval for {:?} on {:?}", hyperbridge_provider.state_machine_id().state_id, id.state_id);
			...
			if current_timestamp
				...
				max_interval
			{
				log::trace!(target: crate::LOG_TARGET, "{:?} -> {:?} Has stalled shutting down", hyperbridge_provider.state_machine_id().state_id, id.state_id);
				return Ok::<_, anyhow::Error>(HealthStat::Restart);
			}
		}
