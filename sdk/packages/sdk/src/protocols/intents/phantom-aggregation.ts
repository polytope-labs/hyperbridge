 ...
 *
 * A bid is the only place a position is ever named, so a consumer that wants to value these
 * between bid windows (an order fill drains a position inside the fill transaction, while wallet
 * and vault balances barely move) has to persist them from here.
 */
...
async function runAggregation(
	...
	for (const bid of bids) {
		...
		try {
			...
			// Reported alongside the balances so a consumer can persist which positions back this
