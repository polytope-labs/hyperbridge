...
An order is a list of legs: leg `i` sells `inputs[i]` for `output.assets[i]`. `inputs` and `output.assets` must be non-empty and the same length, and every output amount must be non-zero. Legs may repeat tokens, for example one pair offered at several prices:
...
An order with predispatch calldata is the one exception: its escrow is swept from the dispatcher and measured per token, so its legs may not repeat an input token.

Tokens are `bytes32` values holding the token address left-padded with zeros. `placeOrder` rejects an input token, and `fillOrder` an output token, whose upper 12 bytes are not zero.
...
`fillOrder(order, options)` accepts per-leg input quotes in `options.inputs`. Each entry names the
matching order input token and the maximum input take. The ratio `outputs[i].amount / inputs[i].amount`
is the solver's exact rate; the quoted output is a debit budget, not a fixed payment.
Every fill must declare input quotes; equal-rate quotes cover ordinary fills. Appending this field changes the selector to
`0x68ddf058`; all fills on an upgraded gateway must use the new tuple. SDK codecs continue to
support historical v1/v2 gateways, but old signed calldata must be requoted after an upgrade.

The contract credits output at the order's rate and releases the corresponding cumulative input
slice. Integer rounding can release less than the quoted maximum; use the SDK's `previewRateFill`
to inspect credit, release, payment and surplus at a given progress. A quote below the order's floor reverts with `RateBelowOrder`;
a positive slice too small to release input reverts with `RateFillTooSmall`.

Every fill pays `max(credited output, ceil(released input × quoted output / quoted input))`.
The credited-output floor prevents integer rounding from underfunding recorded progress.
If earlier solvers leave less work than the quote covers, credit and release shrink under the same
payment rule. Unused ERC-20 budget stays with the solver; unused native value is refunded.
The original signed calldata can be used unchanged. Payment above credited progress is surplus,
split by `surplusShareBps`; calldata orders retain all surplus as protocol dust and must complete
in one fill. Fill events report credited output, so surplus never advances the order twice.

Rate fills use canonical input and output token addresses. Repeated tokens are supported because
escrow and credited progress are tracked independently by leg. Finish or cancel old orders and drain
escrow, fees and pending messages before upgrading; old per-slice rounding debt is not migrated.
No rate or surplus data is added to cross-chain redemption messages or cancellation proofs.

The SDK's `automatic: true` execution option ranks single-leg quotes by rate and multi-leg quotes
by output value normalized to the original input amounts, confirms each slice, and continues until
the order is filled. Normalized amounts describe prices; signed quotes bound funding, and settlement
determines actual payment.

Before sending, it durably records the fully signed UserOperation as pending. On restart it reconciles
the receipt and chain state, and may rebroadcast the identical operation, polling for its receipt as a
first send does (`receiptPolling` on the context). Definitive first-send rejection permits fallback; an
uncertain earlier send blocks another distinct bid until its outcome is established: a receipt, or a
finalized block past the operation's `validUntil` or the order deadline with its nonce unconsumed.
Manual selection remains available.

Every v3 bid requires an updated delegated SolverAccount.
Before signing, the SDK requires release `4` from `version()` on the gateway, configured account
implementation, and live delegation. Gateway release 4 uses FillOptions ABI version 3. The account protects the new and both historical selectors against
submission with the session signature stripped off. Deploy matching gateway modules and redelegate
solvers before enabling v3 bids.
