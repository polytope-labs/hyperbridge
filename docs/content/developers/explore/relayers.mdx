...
chain pays the relayer a reward on top of whatever messaging fees
...
| Mandatory consensus proof delivery (Hyperbridge → EVM destination) | Relayer | On-chain reward on the EVM destination, auto-claimed by tesseract | Destination-chain reward token |
