    ...
    P::Signature: From<MultiSignature> + Send + Sync,
{
    pub async fn new(
        ...
        Ok(BeefyProver {
            ...
            prover,
            client,
            finalized_parachain_height: config.finalized_parachain_height,
    ...
    /// Runs the proving task. Will internally notify the appropriate channels of new epoch justifications as well as
    /// new proofs for ISMP messages.
        ...
        loop {
            ...
            let future = async {
                loop {
                    ...
                    println!("\n\n\nCurrent state {:#?}", self.consensus_state);
                    let (update, latest_height) = self.query_next_finalized_epoch().await?;
                    println!("Latest finalized height {:#?}\n\n\n", latest_height);
                    self.consensus_state.latest_beefy_height = latest_height as u32;

                    if let Some((hash, set_id)) = update {
                        println!("\n\n\nSaw proof for new set_id: {set_id}\n\n\n");
                        // they should be equal
                        assert_eq!(set_id, self.consensus_state.next_authorities.id);
                        let header = self
                            .inner()
                            .relay
                            .rpc()
                        ...
                        {
                            println!(
                                "Fetched commitment: {:?} for block hash {:?}\n\n\n",
                                commitment.commitment, hash
                            );
                            let _proof = self
                                .consensus_proof(commitment.clone(), self.consensus_state.clone())
                                .await?;

                            // todo: put proof into mandatory queue
                            self.consensus_state.latest_beefy_height =
                                commitment.commitment.block_number;
                            self.consensus_state.current_authorities =
                                self.consensus_state.next_authorities.clone();
                            self.consensus_state.next_authorities = {
                                let key =
                                    runtime::storage().beefy_mmr_leaf().beefy_next_authorities();
                            ...
                            println!("New state {:#?}", self.consensus_state);
                            // todo: push consensus state to redis
                        }
                    }

                    let (latest_parachain_height, messages) =
                        self.latest_ismp_message_events().await?;
                    self.finalized_parachain_height = latest_parachain_height;

                    if messages.is_empty() {
                        continue;
                    }

                    let least_height = messages
                        .iter()
                    ...
                    let minimum_height = least_height + self.minimum_finalization_height;
                    if minimum_height > latest_parachain_height {
                        println!(
                            "Waiting for {} blocks before proving finality",
                            minimum_height - latest_parachain_height
                        );
                        let mut beefy_finality_interval =
                            tokio::time::interval(Duration::from_secs(10));

                        loop {
                            beefy_finality_interval.tick().await;
                    ...
                    println!("Proving finality for messages in the range: {least_height}..{minimum_height}");
                    let finalized_hash = self
                        .inner()
                        .relay
                        .rpc()
                        ...
                        .await?;
                    let (commitment, _) =
                        fetch_latest_beefy_justification(&self.inner().relay, finalized_hash)
                            .await?;
                    let _proof = self
                        .consensus_proof(commitment, self.consensus_state.clone())
                        .await?;
                    // todo: put proof into mandatory queue
                }
    ...
    pub async fn latest_ismp_message_events(
        ...
        let latest_height = self.finalized_parachain_height;
        let para_id = extract_para_id(self.client.state_machine_id().state_id)?;
        let finalized_hash = self
            .inner()
            .relay
            ...
            .request::<R::Hash>("beefy_getFinalizedHead", rpc_params![])
            .await?;
        let header = query_parachain_header(&self.inner().relay, finalized_hash, para_id).await?;
        ...
        let events = self
            .query_ismp_events_with_metadata(latest_height, finalized_height)
            .await?;
    ...
    async fn query_ismp_events_with_metadata(
        ...
        let response: HashMap<String, Vec<EventWithMetadata>> = self
            .client
            .client
            .rpc()
            .request("ismp_queryEventsWithMetadata", params)
            .await?;
    ...
    pub async fn query_next_finalized_epoch(
        ...
        let relay_client = self.inner().relay.clone();
        ...
        let mut block_hash_and_set_id = changes
            ...
            .filter(|(_, set_id)| *set_id >= self.consensus_state.next_authorities.id);

        Ok((block_hash_and_set_id.next(), to.number().into()))
    }

    /// Queries the epoch justification for the given epoch boundary
...
mod tests {
    ...
    async fn test_can_produce_proofs() -> Result<(), anyhow::Error> {
        ...
        .await?;
        let consensus_state = prover
            .query_initial_consensus_state(Some(H256::from(hex!(
                "6b33f31d9a5e46d0d735926a29e2293934db4acb785432af3184ede3107aa7b0"
            ))))
            .await?;
        prover.consensus_state = consensus_state;
        let finalized_hash = prover
            ...
            .await?;
        prover.finalized_parachain_height =
            query_parachain_header(&relay_chain, finalized_hash, 4009)
                .await?
                .number as u64;
