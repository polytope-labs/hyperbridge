...
![merkle.webp](./images/merkle.webp)
...
![Proof schema for a single item in the tree. We need log₂(n) = 4 proof nodes.](./images/single1.svg)

Proof schema for a single item in the tree. We need log₂(n) = 4 proof nodes.

![Proof schema for another item in the merkle tree, which uses 3 proof nodes. (note: this tree is unbalanced)](./images/single2.svg)

Proof schema for another item in the merkle tree, which uses 3 proof nodes. (note: this tree is unbalanced)

![A combined merkle multi proof uses only 5 proof nodes, in contrast to the total of 7 proof nodes required for verifying each node individually.](./images/merkle_proof.svg)
...
![Untitled Diagram.drawio (1).svg](./images/Untitled_Diagram.drawio_(1).svg)
...
![Untitled Diagram.drawio (3).svg](./images/Untitled_Diagram.drawio_(3).svg)
