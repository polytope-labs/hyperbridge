...
pub fn verify_header_update(
	...
	// Convert the trusted state to a Tendermint trusted state
	...
	// Convert consensus proof to UntrustedBlockState
	let untrusted_block_state = UntrustedBlockState {
		signed_header: &consensus_proof.signed_header,
		validators: &ValidatorSet::new(consensus_proof.validators.clone(), None),
		next_validators: Some(&ValidatorSet::new(
			consensus_proof.next_validators.clone().unwrap_or_default(),
			None,
		)),
	};

	// Convert options to VerificationOptions
	let trust_threshold =
	...
	let trusting_period = Duration::from_secs(3600); // 1 hour

	let options = Options { trust_threshold, trusting_period, clock_drift };
	let verifier = ProdVerifier::default();
	let now = Time::from_unix_timestamp(current_time as i64, 0)
		.map_err(|e| VerificationError::Invalid(e.to_string()))?;
