...
pub const STANDALONE_CLIENT_ID: [u8; 4] = *b"lone";
...
impl ConsensusClient for StandAloneClient {
    fn verify_consensus(
        &self,
        _host: &dyn IsmpHost,
        _consensus_state_id: ConsensusStateId,
    ...
    fn state_machine(&self, _id: StateMachine) -> Result<Box<dyn StateMachineClient>, Error> {
        match _id {
            StateMachine::Kusama(4500) => Ok(Box::new(MockStateMachineClient)),
            _ => Err(Error::ImplementationSpecific("Invalid state machine".to_string())),
        }
    }
}
...
impl IsmpHost for Host {
    ...
    fn consensus_clients(&self) -> Vec<Box<dyn ConsensusClient>> {
        vec![Box::new(MockClient), Box::new(MockProxyClient), Box::new(StandAloneClient)]
    }
