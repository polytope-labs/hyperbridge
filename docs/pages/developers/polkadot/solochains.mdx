...
```bash
docker run -v /path/to/consensus.toml:/root/consensus.toml polyopelabs/tesseract-consensus:latest --config=/root/consensus.toml log-consensus-state=KUSAMA-4009
```
