...
Rate fills require unique, canonical input and output token identifiers with the current token-keyed
storage layout. A same-chain order whose earlier legacy slices accumulated rounding dust can finish
...
Before signing, the SDK verifies the exact `fillOrderSelector()` on the gateway, configured account
implementation, and live delegation. The account protects the new and both historical selectors against
