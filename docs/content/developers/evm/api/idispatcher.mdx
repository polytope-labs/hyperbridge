...
Interface for dispatching outgoing cross-chain messages. Applications use these methods to send POST requests and GET queries through Hyperbridge.
...
| `timeout` | `uint64` | Timeout in seconds from dispatch time |
| `fee` | `uint256` | Optional relayer fee for delivering the response back to the source chain |
