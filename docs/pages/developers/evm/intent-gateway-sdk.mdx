...
### Apply GET response on source chain (self-relay)

After `HYPERBRIDGE_FINALIZED`, the generator provides `calldata` needed to finalize the refund on the source chain's Handler. Submit it as a transaction to the Handler contract.

```typescript
// 7. Submit the GET response to the Handler on the source chain
// If you don't have it handy, derive `handlerAddress` from the source ISMP Host's hostParams.handler
const handlerAddress = "0xYourSourceChainHandlerAddress" as `0x${string}`;

const calldata = result.value?.data?.metadata?.calldata;

const handlerTx = await walletClient.sendTransaction({
  to: handlerAddress,
  data: calldata,
});

console.log(`Handler execution tx: ${handlerTx}`);
```
...
async function cancelExpiredOrder(order: Order) {
  ...
  // 8. Apply GET response on source chain (self-relay)
  // If you don't have it handy, derive `handlerAddress` from the source ISMP Host's hostParams.handler
  const handlerAddress = "0xYourSourceChainHandlerAddress" as `0x${string}`;
  const calldata = result.value?.data?.metadata?.calldata;
  if (calldata) {
...
- Always simulate destination calldata during fee estimation to ensure `fillOrder` succeeds; the SDK’s estimation includes a simulation step and buffers.
