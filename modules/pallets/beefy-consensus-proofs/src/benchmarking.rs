...
///
/// REGEN REQUIRED: the SP1 public-values format gained a committed `nonce` field, so this
/// fixture (and `FIXTURE_VKEY`) must be regenerated from the rebuilt ELF, with the committed
/// nonce set to the benchmark submitter `[1u8; 32]` so the `nonce == signer` check passes.
