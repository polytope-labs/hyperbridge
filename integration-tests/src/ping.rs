...
use ethers::{
	...
	providers::{Http, Middleware, Provider, ProviderExt}, types::BlockId,
};
use futures::TryStreamExt;
use hex_literal::hex;
use ismp::{consensus::{StateMachineHeight, StateMachineId}, events::Event, host::StateMachine, router::Request};
...
async fn test_ping_get_request() -> anyhow::Result<()> {
	...
	// sepolia host
	let sepolia_host = H160(hex!("fCA0c05bEb9564AC154f55173881B4DD221A18A8"));
	...
	let latest_hyperbridge_height = contract.latest_state_machine_height(4009u32.into()).block(BlockId::Number(ethers::types::BlockNumber::Number(latest_sepolia_height.into()))).call().await?;
	...
	let state = sepolia_client.query_state_machine_commitment(StateMachineHeight { id: StateMachineId {
		state_id: StateMachine::Kusama(4009),
		consensus_state_id: *b"PARA",
	}, height: latest_hyperbridge_height.low_u64() }).await?;
