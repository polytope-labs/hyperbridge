...
impl AccumulateFees {
	...
	pub async fn accumulate_fees(&self, config_path: String, db: String) -> anyhow::Result<()> {
		...
		let tx_payment = TransactionPayment::initialize(&db).await?;
		log::info!(target: "messaging-relayer", "Initialized database");
		...
		stream.for_each_concurrent(None, |delivery| {
			...
			async move {
				...
				let lambda = || async {
					...
					{
						log::info!(target: "messaging-relayer", "No deliveries found in db for {source_chain}->{dest_chain}");
					...
					if let Some(height) = highest_delivery_height_to_dest {
						...
						match height {
							Some(height) => {
								...
								log::info!(target: "messaging-relayer", "Creating withdrawal proof from db for deliveries from {source_chain}->{dest_chain}");
								let proofs = tx_payment
									.create_claim_proof(source_height.into(), height, source.clone(), dest.clone(), &hyperbridge)
									.await?;

								if proofs.is_empty() {
									log::info!(target: "messaging-relayer", "All fees in the database for  {source_chain}->{dest_chain} have been successfully accumulated in a previous attempt")
								} else {
									observe_challenge_period(dest.clone(), Arc::new(hyperbridge.clone()), height).await?;
								}

								log::info!(
									target: "messaging-relayer", "Submitting proofs for {source_chain}->{dest_chain} to hyperbridge"
								);
								for proof in proofs {
									...
									match tx_payment.delete_claimed_entries(proof.commitments).await {
										Err(_) => {
										log::error!(target: "messaging-relayer", "An Error occured while deleting claimed fees from the db, the claimed keys will be deleted in the next fee accumulation attempt");
										}
										_ => {}
									};
								}

								log::info!(target: "messaging-relayer", "Proofs sucessfully submitted");
							},
							None => {
								log::info!(target: "messaging-relayer", "Skipping fee accumulation for {source_chain}->{dest_chain}: state machine update not yet available on hyperbridge");
							},
						}
					}
					...
					if let Some(height) = highest_delivery_height_to_source {
						...
						match height {
							Some(height) => {
								...
								log::info!(target: "messaging-relayer", "Creating withdrawal proof from db for deliveries from {dest_chain}->{source_chain}");
								let proofs = tx_payment
									.create_claim_proof(dest_height.into(), height, dest.clone(), source.clone(), &hyperbridge)
									.await?;

								if proofs.is_empty() {
									log::info!(target: "messaging-relayer", "All fees in the database for  {dest_chain}->{source_chain} have been successfully accumulated in a previous attempt")
								}
								else {
									observe_challenge_period(source.clone(), Arc::new(hyperbridge.clone()), height).await?;
								}
								log::info!(
									target: "messaging-relayer", "Submitting proofs for {dest_chain}->{source_chain} to hyperbridge"
								);
								for proof in proofs {
									hyperbridge.accumulate_fees(proof.clone()).await?;
								...
								log::info!(target: "messaging-relayer", "Proof sucessfully submitted");
							},
							None => {
								log::info!(target: "messaging-relayer", "Skipping fee accumulation for {dest_chain}->{source_chain}: state machine update not yet available on hyperbridge");
							},
						}
					}
				...
				match lambda().await {
					Ok(_) => {},
					Err(e) => log::error!(target: "messaging-relayer", "Fee accumulation for {dest_chain}->{source_chain} failed: {e:?}"),
				}
			}
				...
				async move {
					let lambda = || async {
						...
						if let Err(e) = tx.delete_pending_withdrawals(ids).await {
							tracing::error!(target: "messaging-relayer", "Error encountered while deleting pending withdrawals from the db: {e:?}, \n NOTE: The withdrawal request was successfully delivered.");
						}
						...
						if amount < U256::from(10u128 * 10u128.pow(fee_token_decimals.into())) {
							log::info!(target: "messaging-relayer", "Unclaimed balance on {chain} is less than $10, exiting");
							return Ok::<_, anyhow::Error>(());
						}

						log::info!(
							target: "messaging-relayer", "Submitting withdrawal request to {chain}  for amount ${}",
							amount / U256::from(10u128.pow(fee_token_decimals.into()))
						);
						let results = hyperbridge
							.withdraw_funds(client.clone(), chain)
							.await?;
						log::info!(target: "messaging-relayer", "Request submitted to hyperbridge successfully");
						log::info!(target: "messaging-relayer", "Starting delivery of withdrawal message to {}", chain);
						...
						match deliver_post_request(client.clone(), &hyperbridge, results.clone()).await {
							...
							Err(err) => {
								tracing::info!(target: "messaging-relayer", "Failed to deliver withdrawal request: {err:?}, they will be retried.");
							}
						};
						Ok(())
					};
					...
					match lambda().await {
						Ok(_) => {},
						Err(e) => log::error!(target: "messaging-relayer", "Failed to complete a withdrawal request: {e:?}"),
					}
				}
			})
			.await;
	...
	C: IsmpProvider + HyperbridgeClaim + Clone,
{
	...
	tracing::info!(target: "messaging-relayer", "Auto-withdraw frequency set to {:?}", frequency);
	let min_amount: U256 = (config
		...
		10u128.pow(18))
	.into();
	tracing::info!(target: "messaging-relayer", "Minimum auto-withdrawal amount set to ${:?}", Cost(min_amount));
				...
				async move {
					let lambda = || async {
						...
						if amount < min_amount {
							tracing::info!(target: "messaging-relayer", "Unclaimed balance {amount} on {chain} is < minimum_withdrawal_amount: {min_amount}, exiting");
							return Ok::<_, anyhow::Error>(());
						}

						tracing::info!(
							target: "messaging-relayer", "Submitting withdrawal request to hyperbridge for amount ${} on {chain}",
							amount / U256::from(10u128.pow(fee_token_decimals.into()))
						);
						let results = hyperbridge
							.withdraw_funds(client.clone(), chain)
							.await?;
						tracing::info!(target: "messaging-relayer", "Request submitted to hyperbridge successfully");
						tracing::info!(target: "messaging-relayer", "Starting delivery of withdrawal message to {}", chain);
					...
					match lambda().await {
						Ok(_) => {},
						Err(e) => log::error!(target: "messaging-relayer", "Failed to complete an auto-withdrawal: {e:?}"),
					}
				}
			})
			.await;
...
async fn deliver_post_request<D: IsmpProvider>(
	...
	if max_block > latest_height {
		...
		log::info!(
			target: "messaging-relayer", "Waiting for state machine update that finalizes withdraw height: {}",
			max_block
		);
			...
			match stream.next().await {
				Some(Ok(event)) =>
					if event.latest_height < max_block {
						...
						log::info!(target: "messaging-relayer", "Found a state machine update: {}", event.latest_height);
						break event.latest_height;
					},
				Some(Err(_)) => {
					log::error!(
						target: "messaging-relayer", "An error occured waiting for state machine update from {}, Retrying",
						dest_chain.name()
					);
				},
	...
	log::info!(target: "messaging-relayer", "Querying request proof from hyperbridge at {}", latest_height);
	let proof = hyperbridge
		.query_requests_proof(latest_height, queries, dest_chain.state_machine_id().state_id)
		.await?;
	log::info!(target: "messaging-relayer", "Successfully queried request proof from hyperbridge");
	...
	log::info!(target: "messaging-relayer", "Submitting post request to {}", dest_chain.state_machine_id().state_id);
	...
	while count != 0 {
		...
		{
			log::info!(
					target: "messaging-relayer", "Encountered error trying to submit withdrawal request to {}.\n{e:?}\nWill retry {count} more times.",
					dest_chain.state_machine_id().state_id
				);
			count -= 1;
		} else {
			log::info!(
				target: "messaging-relayer", "Withdrawal message submitted successfully to {}",
				dest_chain.state_machine_id().state_id
			);
			return Ok(());
		}
