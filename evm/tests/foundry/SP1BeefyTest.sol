...
contract SP1BeefyTest is Test {
    ...
    // Trusted consensus state and the matching proof, captured from Polkadot mainnet when the
    // proof was generated against sp1-beefy v1.0.0. The proof is an ABI-encoded SP1BeefyProof
    // (abi_encode_params output; no outer offset).
    function sp1BeefyFixture() internal pure returns (bytes memory state, bytes memory proof) {
        state = hex"";
    ...
    function testVerifySp1Beefy() public view {
        (bytes memory state, bytes memory proof) = sp1BeefyFixture();
    ...
    // one would advance `latestHeight` while replaying an old `nextAuthoritySet`, suppressing the
    // rotation and stranding the client on a set the relay chain has retired.
    function testRejectsStaleMmrLeaf() public {
        (bytes memory state, bytes memory proof) = sp1BeefyFixture();
        ...
        // Point the leaf back at an earlier block, as an attacker replaying a historical leaf would.
        leaf.parentNumber -= 500;

        vm.expectRevert(SP1Beefy.StaleMmrLeaf.selector);
        beefy.verify(state, abi.encode(commitment, leaf, headers, proofBytes, nonce));
    }
}
