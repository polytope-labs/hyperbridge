...
2. `buildSimplexPaymasterData` picks the first configured stablecoin (USDC, then USDT — `configuredFeeTokens`) with a balance of at least one whole token (`selectToken`). When none qualifies it returns the balances it read instead of paymaster data, and step 1 records them as `simplex: solver USDC balance 0 < 1000000, USDT balance 0 < 1000000`, or `simplex: no fee token configured`. Otherwise:
...
`DelegationService.setupDelegation(chain)` first asks `resolvePendingPermit2Approval` whether the chain's fee token has no Permit2 allowance yet, and — when it does not — whether that token implements EIP-2612 (`permitCapable`). The answer picks the carrier for the approve, because until it lands nothing can be charged through Permit2:
