...
An order with predispatch calldata must be single-leg: its escrow is swept from the dispatcher and measured per token, and every leg holds the same token.
