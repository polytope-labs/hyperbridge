...
impl IsmpHost for BscPosHost {
    async fn consensus_notification(
        ...
        // We only try to yield new consensus updates when the queried consensus state is different
        // from the state maintained in the stream's internal state.
        ...
        let stream = stream::unfold(
            (interval, None),
            move |(mut interval, prev_consensus_state)| {
                ...
                async move {
                    ...
                    let lambda = || async {
                        ...
                        let (update, cs_state) =
                            consensus_notification(&client, counterparty, block).await?;
    ...
    async fn query_initial_consensus_state(&self) -> Result<Option<CreateConsensusState>, Error> {
        let initial_consensus_state = self
            .get_consensus_state::<KeccakHasher>(self.evm.ismp_host)
            .await?;
