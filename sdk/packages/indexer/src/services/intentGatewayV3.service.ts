...
export class IntentGatewayV3Service {
	 ...
	 *
	 * The current event names the solver. The legacy shape names no filler — the gateway records the
	 * beneficiary when `_withdraw` finalizes — so its handler resolves it via `filledBeneficiary`. A
	 * release whose beneficiary is not a known provider publishes nothing.
	 */
