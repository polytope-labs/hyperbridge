...
impl<T> IsmpModule for Pallet<T>
{
	fn on_accept(&self, request: Post) -> Result<(), ismp::Error> {
        ...
        // You can also dispatch a post response
...
## Integrating the module with the runtime
   ...
   impl IsmpRouter for ModuleRouter {
      fn module_for_id(&self, id: Vec<u8>) -> Result<Box<dyn IsmpModule>, Error> {
          return match id.as_slice() {
              ...
              id if id == EXAMPLE_MODULE_ID.to_bytes().as_slice() => Ok(Box::new(pallet_example::Pallet::<Runtime>::default()))
              // ... other modules
