...
async fn on_connect(socket: SocketRef, Data(data): Data<Message>, state: State<SharedAppState>) {
	tracing::info!(target: "messaging-telemetry", "socket connected {}", socket.id);
	...
	if matches!(signature, Ok(true)) {
		tracing::info!(target: "messaging-telemetry", "Authenticated {} successfully", socket.id);
		// add to global state
		state.write().await.insert(socket.id, data);
	} else {
		tracing::info!(target: "messaging-telemetry", "Disconnecting unauthorized {}", socket.id);
	...
	socket.on_disconnect(
		|socket: SocketRef, reason: DisconnectReason, state: State<SharedAppState>| async move {
			tracing::info!(
				target: "messaging-telemetry", "Socket {} on ns {} disconnected, reason: {:?}",
				socket.id,
				socket.ns(),
				reason
			);
...
async fn main() -> Result<(), anyhow::Error> {
	...
	tracing::info!(target: "messaging-telemetry", "Starting server on port: {port}");
