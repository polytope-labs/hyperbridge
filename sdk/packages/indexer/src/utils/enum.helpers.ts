 ...
 */
export const convertEnumListStringToArray = (enumListString: string): Array<string> => {
	// Remove the wrapping brackets, e.g. {SEPOLIA, BSC} ==> SEPOLIA, BSC
	if (["{", "["].includes(enumListString[0]) && ["}", "]"].includes(enumListString[enumListString.length - 1])) {
 ...
 * @see https://stackoverflow.com/questions/18234946/postgresql-insert-into-an-array-of-enums
 */
export const convertArrayToEnumListString = (enumsArray: Array<string>): string => {
