...
## The path to becoming a Collator: A Step-by-Step Guide
...
1. Navigate to `Developer -> Extrinsics` in the Polkadot UI.
2. Select your main account(the one with the reputation score).
3. Choose the `session` pallet and the `setKeys` extrinsic.
4. Paste the entire hex string from the previous step into the `keys` field.
5. Leave the `proof` field empty (0x00).
...
1. Navigate to `Developer -> Extrinsics` in the Polkadot-JS UI.
2. Select your main account.
3. Choose the `collatorSelection` pallet and the `registerAsCandidate` extrinsic.
