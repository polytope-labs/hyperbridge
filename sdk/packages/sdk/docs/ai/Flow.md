...
`signRawHash` is declared on the interface but never invoked here (`signMessage`, and `signTypedData`'s chain-id argument, were removed on 2026-08-18 for the same reason); `GasEstimator`'s `signMessage` call is viem's method on a locally derived account, unrelated to `SigningAccount`.
