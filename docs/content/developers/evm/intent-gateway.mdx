...
description: The Intent Gateway is the core smart contract that enables cross-chain intent-based swaps on Hyperbridge. It allows users to place orders on one chain to receive tokens on another chain, with fillers providing liquidity and earning fees for executing these orders.
---
