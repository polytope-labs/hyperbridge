...
mod tests {
	...
	fn generate_ecdsa() -> Result<(), anyhow::Error> {
		...
		for (total, major, _minor, name) in stuff {
			...
			let leaf_hashes = leaves.iter().map(|l| keccak_256(l.as_bytes())).collect::<Vec<_>>();
			...
			let votes = signatures
				...
				.map(|(i, (sig, key))| {
					...
					Vote { key, signature, index: H160::from_low_u64_be(i as u64) }
				})
				.collect();
			...
			let params = ProverParams { votes, siblings, msg, root };
			std::fs::write(
