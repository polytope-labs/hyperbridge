---
title: IHandlerV2
description: API reference for IHandlerV2 - interface for verifying and processing ISMP messages.
---

# IHandlerV2

Interface for verifying and processing ISMP messages. The handler validates consensus proofs and state proofs before dispatching messages to applications. Supports batching multiple handler calls into a single transaction via `batchCall`.
...
| `nonce` | `uint64` | Unique nonce for this request on the source chain |
| `from` | `bytes` | Address of the application that initiated this query |
