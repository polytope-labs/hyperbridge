...
`fillOrderAtRate(order, options, inputs)` adds a per-leg input quote while retaining the existing
`fillOrder(order, options)` selector and `FillOptions` struct. Each `inputs[i]` names the matching
order input token and the maximum input the solver takes for `options.outputs[i]`. Empty inputs
use ordinary order-rate settlement.
...
or cancel through the existing methods, but rate settlement reverts with `LegacyRateAccounting`.
...
slice, and continues until the order is filled. An uncertain submission stops execution and reports
its error; the attempted UserOp is persisted to prevent replay on resume. Manual bid selection
remains available for multi-leg rate quotes, and existing multi-leg legacy best-bid execution
continues to work. Simplex
uses quoted takes when both the gateway and delegated SolverAccount report `supportsRateFills()`.
Rate bids must use the updated account: its validation recognizes both selectors and rejects a
public bid submitted with its session signature stripped off.
...
When a solver provides more tokens than required, the excess is split according to `surplusShareBps` (100% to protocol if the order has calldata). For ordinary `fillOrder`, surplus only applies on the first fill of each output token pair. For `fillOrderAtRate`, surplus applies to each quoted slice.
