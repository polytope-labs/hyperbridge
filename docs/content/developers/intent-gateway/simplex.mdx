...
The optional **`spreadBps`** field (basis points) sets a symmetric spread around the Uniswap V4 mid: for example, `spreadBps = 50` applies a 0.5% spread (bid is 0.5% above mid in exotic-per-USD terms, ask 0.5% below). Defaults to `0` if omitted.
...
**Uniswap V4** — Decreases liquidity from concentrated Uniswap V4 positions via PositionManager NFTs. Roughly **0.10%** slippage tolerance on the remove-liquidity path, 30-minute deadline. Only the `tokenId` (the ERC-721 position NFT ID) is required; pool details are read on-chain at startup.
...
**Aerodrome** — Withdraws LP from Aerodrome pools on Base. Supports both volatile and stable pools. 0.3% slippage tolerance. The `gauge` field is optional — if provided, staked LP is withdrawn from the gauge before burning.
