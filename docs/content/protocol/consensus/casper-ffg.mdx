...
BLS signatures<sup>[1]</sup> enable consensus proofs that are very efficient to verify as it supports both public key and signature aggregation. So a verifier only needs to verify a single aggregate signature rather than $n$ signatures.
...
What about verifying these terms? This is possible with KZG multi-proofs<sup>[3]</sup>. Dankrad's article on KZG commitments outlines that to verify batch KZG proofs, the verifier needs to compute some Lagrange bases themselves. However, the complexity for the bases is now reduced to $O(2(I - 1))$. In our case, the prover has already supplied those Lagrange bases. Let's define $I(x)$ as follows:
...
![Blocks b1 & b2 are finalized, while b3 is only justified.](./images/Screenshot_2023-05-09_at_3.12.19_PM.png)
...
The _Accountable Light Client Systems for PoS Blockchains_<sup>[5]</sup> paper by Web3 Foundation researchers presents a SNARK that can verify whether the constituent public keys in an aggregate BLS public key exist as a subset of a list of potential signers. Using this SNARK the verifier only needs to maintain a KZG commitment to the BLS public keys of all potential signers.
...
![Screenshot 2023-05-10 at 11.52.59 AM.png](../images/sudoku.png)
...
<sup>[1]</sup> [Dan Boneh; Ben Lynn & Hovav Shacham. "Short Signatures from the Weil Pairing".](https://www.iacr.org/archive/asiacrypt2001/22480516.pdf)

<sup>[2]</sup> [Dan Boneh, Manu Drijvers, and Gregory Neven. "Compact Multi-Signatures for Smaller Blockchains"](https://eprint.iacr.org/2018/483.pdf)

<sup>[3]</sup> [Dankrad Feist, Kate Polynomial Commitments](https://dankradfeist.de/ethereum/2020/06/16/kate-polynomial-commitments.html)

<sup>[4]</sup> [Vitalik Buterin and Virgil Griffith. "Casper the Friendly Finality Gadget"](https://arxiv.org/pdf/1710.09437.pdf)

<sup>[5]</sup> [Oana Ciobotaru, Fatemeh Shirazi, Alistair Stewart , and Sergey Vasilyev. "Accountable Light Client Systems for PoS Blockchains"](https://eprint.iacr.org/2022/1205.pdf)

<sup>[6]</sup> [Combining Ghost and Casper](https://arxiv.org/pdf/2003.03052.pdf)
