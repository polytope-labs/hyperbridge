...
2. `buildSimplexPaymasterData` picks the first configured stablecoin (USDC, then USDT — `configuredFeeTokens`) with a balance of at least one whole token (`selectToken`). When none qualifies it returns the balances it read instead of paymaster data, and step 1 records them as `simplex: solver USDC balance 0 < 1000000, USDT balance 0 < 1000000` — the Circle branch's own form — or `simplex: no fee token configured`. Otherwise, the authorization mode:
   - `tokenSupportsPermit` (probes `version()`): `buildPermitMode` signs an EIP-2612 permit (`permit.ts`, `deadline = maxUint256`) and packs mode `0x00` (150 bytes). An existing allowance to the paymaster is not consulted; the contract has no mode that spends one.
   - Otherwise `permit2 = configService.getPermit2Address(chain)` must be configured and `paymasterSupportsPermit2` (a cached `PERMIT2()` read on the paymaster) must hold, else the builder throws an error naming the reason; `buildPaymasterAndData` catches it, warns, records it as the Simplex skip reason and moves on to Circle or the deposit. Then:
     - Permit2 allowance below $5: bootstrap with `sendFundedApprove`: pre-check the native balance against the whole sequence, reset a stale non-zero allowance to zero first (USDT rule), then `approve(Permit2, max)` from the solver EOA, up to two txs, each waiting two confirmations.
     - `buildPermit2Mode`: random nonce, `deadline = now + PERMIT2_DEADLINE_SECONDS`, `signPermit2Transfer` (`permit2.ts`, canonical v4 typed data, 65-byte signature) and pack mode `0x02` (182 bytes) with `VERIFICATION_GAS_LIMIT_PERMIT2`.
