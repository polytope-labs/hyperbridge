...
| [`IsmpDispatcher`](/protocol/ismp/dispatcher) | It implements `IsmpDispatcher` allowing it to dispatch requests and responses to Hyperbridge or any other connected state machine. Optional relayer fees attached at dispatch time are escrowed for the relayer that delivers the message; see [Fees](/developers/polkadot/fees) for details. |
