...
import {
	...
	fetchPoolCurrencyDecimals,
} from "@/funding/uniswapV4/v4PoolCurrency";
...
export class UniswapV4FundingPlanner implements FundingVenue {
	...
	static validateConfig(
		...
		for (const pos of positions) {
			if (!pos.chain?.trim()) {
				throw new Error(
					"Each UniswapV4 outputFunding position must have a non-empty 'chain' (e.g. EVM-8453)",
				);
			}
			if (!pos.tokenId) {
	...
	async initialise(solver: HexString): Promise<void> {
		for (const [chain, positions] of Object.entries(
			this.config.positionsByChain,
		)) {
			logger.info(
				{ chain, positionCount: positions.length, solver },
				"UniswapV4 initialising chain",
			);
			const positionManager =
				this.configService.getUniswapV4PositionManagerAddress(chain);
			const poolManager =
				this.configService.getUniswapV4PoolManagerAddress(chain);
			const stateView = this.configService.getUniswapV4StateViewAddress(chain);

			if (!positionManager || positionManager === "0x") {
				throw new Error(
					`UniswapV4 PositionManager not configured for chain ${chain}`,
				);
			}
			if (!poolManager || poolManager === "0x") {
				throw new Error(
					`UniswapV4 PoolManager not configured for chain ${chain}`,
				);
			}
			if (!stateView || stateView === "0x") {
				throw new Error(
					`UniswapV4 StateView not configured for chain ${chain}`,
				);
			}

			logger.info(
				{ chain, positionManager, poolManager, stateView },
				"UniswapV4 addresses resolved",
			);
	...
	async refresh(chain?: string): Promise<void> {
		if (chain) {
			...
			await Promise.all(
				Array.from(this.stateByChain.values()).map((s) => s.refresh()),
			);
		}
	}
	...
	async planWithdrawalForToken(
		...
		for (const pos of candidates) {
			...
			// Use binary search to find the minimal liquidity that covers `remaining`
			const neededLiq = this.findLiquidityForDeficit(
				state,
				pos,
				isToken0,
				remaining,
