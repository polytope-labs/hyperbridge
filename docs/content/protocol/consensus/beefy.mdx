...
Validators are essentially signing the latest mmr root. These signatures can be found in the [justifications](https://spec.polkadot.network/sect-finality#defn-grandpa-justification) portion of the relay chain block. With signatures over the latest mmr root, consensus clients must:
 ...
 - Verify the latest leaf in the mmr tree, so they can be aware of the latest finalized block number and hash. It must be the *latest* leaf: an mmr is append-only, so every historical leaf also has a valid inclusion proof against the signed root, and a client that accepts an older one can be advanced in height while its authority set is held back. The leaf is pinned by requiring `parent_number + 1 == block_number`.
