...
```typescript lineNumbers title="client.ts" icon=typescript
import { RequestStatus, TimeoutStatus, EvmHostABI } from "@hyperbridge/sdk"
...
for await (const status of indexer.getRequestStatusStream(commitment)) {
  ...
  switch (status.status) {
    ...
    case RequestStatus.HYPERBRIDGE_FINALIZED:
      ...
      // Resolve the source Handler address via the Host.
      // Don't hardcode it — governance can rotate the Handler.
      const sourceHost = getContract({
        address: sourceHostAddress,
        abi: EvmHostABI,
        client: sourcePublicClient,
      })
      const { handler: handlerAddress } = await sourceHost.read.hostParams()

      try {
        // Submit the calldata as-is. The SDK already picked the right selector
        // (handleGetResponses on HandlerV1, batchCall on HandlerV2).
...
When the request reaches `HYPERBRIDGE_FINALIZED` status, the metadata includes calldata for the source chain's Handler contract containing the queried data and cryptographic proofs. The SDK picks the right function selector for you: bare [`handleGetResponses(IHost, GetResponseMessage)`](/developers/evm/api/ihandler#handlegetresponses) on HandlerV1 sources, or a `batchCall(bytes[])` that bundles `handleConsensus` + `handleGetResponses` on HandlerV2 sources. Either way, submit the raw calldata as a plain transaction to the Handler address. You'll need gas tokens on the source chain for submission. Host contract addresses for each network are listed in the [mainnet](/developers/evm/contract-addresses/mainnet) and [testnet](/developers/evm/contract-addresses/testnet) deployment pages.
...
At this point, Switch to [`getRequestTimeoutStream()`](/developers/sdk/api/ismp-client#getrequesttimeoutstream) to track this timeout processing through its stages. See the [timeout tracking guide](/developers/sdk/tracking/timeouts) for complete examples and best practices.
...
### Direct Query (Without IsmpClient)

Query GET request status directly without creating an IsmpClient:
...
| `calldata` | `HexString` | Optional. Only present when status is `HYPERBRIDGE_FINALIZED`. Contains encoded transaction data for the source Handler contract — either `handleGetResponses` (HandlerV1) or a `batchCall` wrapping `handleConsensus` + `handleGetResponses` (HandlerV2) |
