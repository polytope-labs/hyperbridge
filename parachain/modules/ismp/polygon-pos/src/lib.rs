...
use polygon_pos_verifier::{
    primitives::CodecHeader, verify_polygon_header, EcdsaRecover, VerificationResult,
};
...
pub struct Chain {
    ...
    pub validators: Option<BTreeSet<H160>>,
    ...
    for PolygonClient<T, H>
{
    fn verify_consensus(
        ...
        for header in consensus_update {
            ...
            // Header must be a descendant of the latest finalized header or one of the chain forks
            if parent_hash == consensus_state.finalized_hash {
                ...
                let validators =
                    chain.validators.as_ref().unwrap_or(&consensus_state.finalized_validators);
        ...
        if let Some(mut longest_chain) = longest_chain {
            ...
            // todo: Finalized validator selection is wrong
            if let Some(validators) = longest_chain.validators.clone() {
...
impl EcdsaRecover for SigVerifier {
    fn recover(sig: [u8; 65], msg: H256) -> Result<H160, anyhow::Error> {
        let address = sp_io::crypto::secp256k1_ecdsa_recover_compressed(&sig, &msg.0)
            .map_err(|e| anyhow!("Signature verification failed"))?;
