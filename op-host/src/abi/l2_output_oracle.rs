...
pub mod l2_output_oracle {
    ...
    impl<M: ::ethers::providers::Middleware> L2OutputOracle<M> {
        ...
        pub fn initialize(
            ...
            self.0
                .method_hash(
                    [228, 163, 1, 22],
                    (starting_block_number, starting_timestamp),
                )
                .expect("method not found (this should never happen)")
        }
    ...
    ///Container type for all return fields from the `latestBlockNumber` function with signature `latestBlockNumber()` and selector `0x4599c788`
    #[derive(
        Clone,
        ::ethers::contract::EthAbiType,
        ::ethers::contract::EthAbiCodec,
