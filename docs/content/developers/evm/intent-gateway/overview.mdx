...
matching order input token and the maximum input the solver takes for `options.outputs[i]`.
Empty inputs use ordinary order-rate settlement. Appending this field changes the selector to
...
to inspect the actual release. A quote below the order's floor reverts with `RateBelowOrder`;
a positive slice too small to release input reverts with `RateFillTooSmall`.

If earlier solvers leave less work than a signed quote covers, the contract caps credited progress
and scales output against the actual input release, preserving the quote's rate. The original signed
calldata can be used unchanged. Output above credited progress is surplus on every rate fill,
...
escrow and credited progress are tracked independently by leg. A same-chain order whose earlier legacy slices accumulated rounding dust can finish
with empty input quotes or cancel, but rate settlement reverts with `LegacyRateAccounting`.
No rate or surplus data is added to cross-chain redemption messages or cancellation proofs.

The SDK's `automatic: true` execution option walks single-leg bids in rate order, confirms each
slice, and continues until the order is filled. Before sending, it durably records the fully signed
UserOperation as pending. On restart it reconciles the receipt and chain state, and may rebroadcast
the identical operation. Definitive first-send rejection permits fallback; an uncertain earlier send
blocks another distinct bid until its outcome is established. Manual selection remains available
for multi-leg rate quotes, and existing multi-leg order-rate execution continues to work.

Every v3 bid, including an ordinary fill with empty inputs, requires an updated delegated SolverAccount.
...
When a solver provides more tokens than required, the excess is split according to `surplusShareBps` (100% to protocol if the order has calldata). For `fillOrder` with empty inputs, surplus only applies on the first fill of each output token pair. With explicit input quotes, surplus applies to each quoted slice.
