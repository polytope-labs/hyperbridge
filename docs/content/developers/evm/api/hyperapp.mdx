...
Dispatches a POST request using the fee token for payment, handling approvals automatically. The payer is read from `request.payer`.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `request` | `DispatchPost` | The POST request to dispatch (includes `payer` field) |

**Returns:** Commitment hash identifying the dispatched request

**Behavior:**
1. If `request.payer != address(this)`, transfers fee tokens from payer to this contract
2. Approves Host to spend the fee tokens
3. Dispatches the request to Host
...
Dispatches a GET request using the fee token for payment, handling approvals automatically. The payer is read from `request.payer`.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `request` | `DispatchGet` | The GET request to dispatch (includes `payer` field) |
