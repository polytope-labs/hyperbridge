...
async fn on_connect(socket: SocketRef, Data(data): Data<Message>, state: State<SharedAppState>) {
	tracing::info!(target: crate::LOG_TARGET, "socket connected {}", socket.id);
	...
	if matches!(signature, Ok(true)) {
		tracing::info!(target: crate::LOG_TARGET, "Authenticated {} successfully", socket.id);
		// add to global state
		state.write().await.insert(socket.id, data);
	} else {
		tracing::info!(target: crate::LOG_TARGET, "Disconnecting unauthorized {}", socket.id);
	...
	socket.on_disconnect(
		|socket: SocketRef, reason: DisconnectReason, state: State<SharedAppState>| async move {
			tracing::info!(
				target: crate::LOG_TARGET, "Socket {} on ns {} disconnected, reason: {:?}",
				socket.id,
				socket.ns(),
				reason
			);
...
async fn main() -> Result<(), anyhow::Error> {
	...
	tracing::info!(target: crate::LOG_TARGET, "Starting server on port: {port}");
