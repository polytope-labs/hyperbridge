...
When funding venues (Uniswap V4) are configured, HyperFX can derive bid/ask prices directly from on-chain pool state instead of relying on static price curves. Prices are read from the pool's current tick every 6 seconds and cached in memory.

With venue pricing enabled, the `bidPriceCurve` and `askPriceCurve` fields become **optional**. The `spreadBps` field controls the symmetric spread applied around the venue-derived mid price — for example, `spreadBps = 50` applies a 0.5% spread (bid is 0.5% above mid, ask is 0.5% below mid).

When both static curves and funding venues are configured, **venue prices always take precedence**. The static curves serve as the initial fallback until the first venue price refresh completes. After that, each 6-second poll overwrites the bid/ask policies with the latest pool price.

Uniswap V4 venue pricing uses pools that pair the exotic token with **USDC or USDT** (addresses from your chain config). Pools quoted in other assets (for example WETH) are not used for live USD pricing. When multiple positions exist for the same exotic token on a chain, the most-liquid pool's price is used.
...
<Callout type="info">
At least one of bid/ask price curves or funding venues must be configured. If neither is provided, Simplex will fail at startup with a validation error.
</Callout>
...
**Uniswap V4** — Decreases liquidity from concentrated Uniswap V4 positions via PositionManager NFTs. 0.30% slippage tolerance, 30-minute deadline. Only the `tokenId` (the ERC-721 position NFT ID) is required; pool details are read on-chain at startup.
...
<Callout type="info">
Both funding venues can be configured simultaneously. They are initialized at startup and refreshed on-demand — state is fetched fresh from the chain immediately before each withdrawal plan, ensuring calculations use the latest liquidity and price data. Simplex queries all configured venues and aggregates liquidity across multiple positions or pools if needed. Funding venues also provide live exotic token prices derived from pool state, which HyperFX uses to set bid/ask rates automatically (see **Live DEX Pricing** above).
</Callout>
