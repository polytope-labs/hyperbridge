...
struct TeleportParams {
	...
	bytes data;
	// Amount of provided feeToken to swap for Hyperbridge fee token
	uint256 amountInMax;
}
...
contract TokenGateway is BaseIsmpModule {
	...
	function teleport(TeleportParams memory teleportParams) public payable {
		...
		if (_erc20 != address(0) && !teleportParams.redeem) {
			...
			if (msg.value >= teleportParams.amount && _erc20 == WETH) {
				// wrap native token
		...
		if (feeToken != teleportParams.feeToken) {
			address uniswapV2 = IIsmpHost(_params.host).uniswapV2Router();
			if (msg.value >= teleportParams.amountInMax && teleportParams.amountInMax > 0) {
				// user has provided the native token for swapping, we swap exactly amountInMax
				address[] memory path = new address[](2);
				path[0] = IUniswapV2Router02(uniswapV2).WETH();
				path[1] = feeToken;
				IUniswapV2Router02(uniswapV2).swapETHForExactTokens{value: teleportParams.amountInMax}(
					fee,
					path,
					address(this),
					block.timestamp
				...
				// user wants to swap with some other token
				...
				IUniswapV2Router02(uniswapV2).swapTokensForExactTokens(
					fee,
					teleportParams.amountInMax,
					path,
					address(this),
