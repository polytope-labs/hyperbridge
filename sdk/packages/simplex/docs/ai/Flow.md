...
6. `Simplex.stop()` calls `close()`, which closes both databases, skipping any whose `isOpen`
   is already false. Nothing registers a `process.on("exit")` hook — an earlier version did,
   which leaked a listener per instance and made two fillers in one process race each other's
   teardown. Both files are crash-safe unclosed.
