...
encoded for. `supportsRateFills` applies the same check to the gateway and a `SolverAccount`, and
a bid is signed or counted only when the gateway, the solver's live delegation and the configured
account implementation all report release 3.
