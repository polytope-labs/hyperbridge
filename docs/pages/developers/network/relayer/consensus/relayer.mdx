...
The Hyperbridge consensus relayer (tesseract) can be obtained through a variety of ways. For now only release artifacts for x86 linux environments are officially distributed. You can also build the relayer from source if you prefer.
...
You can follow the steps below if you'd prefer to build the tesseract consensus relayer from source:

::::steps

### Install dependencies

Building the Tesseract consensus relayer requires some dependencies
...
cd ./hyperbridge
git checkout tesseract-consensus-${LATEST_TAG}
```

### Build the tesseract consensus relayer

```bash
cargo build --release -p tesseract-consensus
```

The Tesseract consensus relayer will now be available at `target/release/tesseract-consensus`, You can move the binary to your `$PATH` so you can run it directly.
...
docker run -d \
  ...
  #--base='<STRING_REPRESENTATION_OF_STATEMACHINE>'
```
...
# Configuration for the Sepolia testnet (L1).
[sepolia]
...
signer = "SIGNER-KEY"
# (Optional) A list of Layer 2 state machine identifiers that are hosted by this L1 chain.
layer_twos = ["EVM-421614", "EVM-11155420", "EVM-84532"]
...
[optimism.host]
# The address of the DisputeGameFactory contract on the L1.
