...
import assetAccountBalance from "./images/asset_account_balance.png";
import sampleSubscanGenerated from "./images/sample_subscan_generated.png";
import rotateKeys from "./images/rotate_keys.png";
import setCollatorKeys from "./images/set_collator_keys.png";
import registerController from "./images/register_controller.png";
import registerAsCandidateWithStash from "./images/register_as_candidate_with_stash.png";
...
- **Running the Fisherman Task**: Every collator runs an in-node fisherman that monitors each supported L2 across multiple independent RPC
...
**Outcome**: For every `$BRIDGE` reward your Controller account receives, the corresponding pallet (`pallet-messaging-incentives`, `pallet-consensus-incentives`, or `pallet-beefy-consensus-proofs`) automatically mints an equivalent amount of Reputation Asset to that same Controller account. You can check your Reputation Asset balance in the Polkadot-JS UI by going to `Developer > Chain State > Assets > Account`.

<figure>
  <img src={assetAccountBalance.src} alt="Reputation Asset Account Balance   " />
...
<figure>
  <img src={sampleSubscanGenerated.src} alt="Sample Key Generation with subscan  " />
...
A minimal mainnet collator config looks like this. The `[hyperbridge]` table holds the substrate host fields directly, and each L2
is a top-level `[<chain>]` table with `type = "evm"`. The `[relayer]` section is optional for collators and can be omitted, since
the fisherman task is the only role the collator path spawns.

```toml lineNumbers
# Hyperbridge parachain host config. Required.
[hyperbridge]
state_machine = "POLKADOT-4009"
hashing = "Keccak"
rpc_ws = "ws://127.0.0.1:9933"
# Your Controller account's private key
signer = "0xe12f8d9bf570f710db4197853a306a20259b82b5a394fa06c64e8fa3037bcaa6"

# Each of the five mainnet L2s must be present, with at least two distinct-host
# rpc_urls. Three is strongly recommended. URLs that share a host are rejected.
[arbitrum]
type = "evm"
rpc_urls = [
...
`state_machine`, `ismp_host`, and `consensus_state_id` on each L2 table are derived from the chain's `eth_chainId` against the
first RPC URL, so a minimal collator config only needs `type` and `rpc_urls`. Operators who also run a relayer alongside the
collator can add a `[relayer]` section and `[<chain>.consensus]` sub-tables in the same file; those don't affect fisherman
validation.
...
<figure>
  <img src={setCollatorKeys.src} alt="Setting session keys" />
...
<figure>
  <img src={registerController.src} alt="Registering Your Controller" />
...
<figure>
  <img src={registerAsCandidateWithStash.src} alt="Registering as candidate using Stash Account" />
...
When you are ready to stop collating and reclaim your candidacy bond, the exit follows a two-step unbonding process managed by `pallet-collator-manager`. The direct `collatorSelection.leaveIntent` extrinsic is blocked on Hyperbridge — all exits must go through this flow.

The seven day delay between starting and completing unbonding exists so the protocol has time to act on any detected misbehaviour — such as a fisherman veto — before the operator can walk away with their stake.

### Step 1: Start unbonding from your Stash account

Call `collatorManager.unbond` from your **Stash** account. This marks your Stash as unbonding: it stops being selected in future sessions and records the block at which your bond becomes withdrawable (current block plus seven days of blocks).

- Navigate to `Developer -> Extrinsics` in the Polkadot-JS UI.
- Select your **Stash** account.
- Choose the `collatorManager` pallet and the `unbond` extrinsic.
- Sign and submit.

After this transaction, your Stash is removed from session selection. Your node can be shut down once the current session ends.

### Step 2: Withdraw your bond after the delay

Once seven days have passed, call `collatorManager.withdrawUnbonded` from your **Stash** account. This drops your candidacy from `pallet-collator-selection`, unreserves your bond, and clears the stash–controller pairing.

- Navigate to `Developer -> Extrinsics` in the Polkadot-JS UI.
- Select your **Stash** account.
- Choose the `collatorManager` pallet and the `withdrawUnbonded` extrinsic.
- Sign and submit.

The transaction will fail with `UnbondingPeriodNotElapsed` if the seven days have not yet passed. Once it succeeds, your bond is fully released and the account is no longer registered as a candidate.
