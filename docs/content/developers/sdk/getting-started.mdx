...
`PolkadotHubChain.create(evmRpcUrl, substrateRpcUrl)` mirrors [`EvmChain.create`](/developers/sdk/api/evm-chain): it resolves chain id and `IsmpHost` from known deployments, then attaches your Substrate endpoint for proofs.

```typescript lineNumbers title="client.ts" icon=typescript
import { PolkadotHubChain } from "@hyperbridge/sdk"

const polkadotHub = await PolkadotHubChain.create(
  "https://your-evm-endpoint.example",
  "https://your-substrate-endpoint.example",
)
```
