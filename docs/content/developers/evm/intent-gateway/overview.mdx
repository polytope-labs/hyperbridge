...
Tokens are `bytes32` values holding the token address left-padded with zeros. `placeOrder` rejects any input or output token whose upper 12 bytes are not zero.
