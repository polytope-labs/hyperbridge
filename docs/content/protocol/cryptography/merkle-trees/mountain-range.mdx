...
description: Merkle mountain ranges<sup>[1]</sup> are an improvement over conventional merkle trees for growing, potentially unbounded lists.
---

import mmrGrowth from "./images/mmr-01.svg";
import mmrSubtrees from "./images/mmr-02.svg";

# Merkle Mountain Ranges

Merkle mountain ranges<sup>[1]</sup> are an improvement over conventional merkle trees for growing, potentially unbounded lists. Where conventional merkle tree constructions over growing lists prove very inefficient to compute, as all nodes in the tree must be recomputed. Merkle mountain ranges amortise this cost by growing subtrees incrementally and merging subtrees at the same height, rather than growing the full tree.


<figure>
  <img src={mmrGrowth.src} alt="MMR tree" />
...
First we define the _termial function $n?$ <sup>[2]</sup>_ as the sum:
...
<figure>
  <img src={mmrSubtrees.src} alt="MMR tree" />
...
<sup>[1]</sup> P. Todd. Merkle Mountain Ranges. GitHub, 2012.
[https://opentimestamps.org](https://opentimestamps.org/).

<sup>[2]</sup> Donald Knuth in *The Art of Computer Programming.*
