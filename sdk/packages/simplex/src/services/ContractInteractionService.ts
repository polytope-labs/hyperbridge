...
export class ContractInteractionService {
	...
	async preparePhantomBidUserOp(
		...
		const sdkHelper = await this.getIntentGateway(order.source, order.destination)
		const client = this.clientManager.getPublicClient(order.destination)
		const version = await getFillOptionsVersion(
			client as any,
			this.configService.getIntentGatewayAddress(order.destination),
		)
		const quotedInputs =
			inputs ??
			(version === 3
				? order.inputs.map((input, index) => ({
						...input,
						amount: fillerOutputs[index]?.amount === 0n ? 0n : input.amount,
					}))
				: [])
