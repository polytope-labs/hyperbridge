...
pub async fn handle_message_submission(
	...
	log::trace!(target: "messaging-tron", "handle_message_submission called with {} messages", messages.len());

	let receipts = submit_messages(client, messages.clone()).await?;
	log::trace!(target: "messaging-tron", "submit_messages returned {} receipts", receipts.len());

	let height = client.evm.client.get_block_number().await.unwrap_or(0);
	log::trace!(target: "messaging-tron", "Current block height: {}", height);
	...
	for msg in &messages {
		match msg {
			Message::Request(req_msg) => {
				log::trace!(target: "messaging-tron", "Processing Request message with {} posts", req_msg.requests.len());
				for post in &req_msg.requests {
					let req = Request::Post(post.clone());
					let commitment = hash_request::<Hasher>(&req);
					log::trace!(target: "messaging-tron", "Request commitment: {:?}", commitment);
					if receipts.contains(&commitment) {
						log::trace!(target: "messaging-tron", "Adding receipt for request commitment {:?}", commitment);
			...
			Message::Response(ResponseMessage {
				...
				log::trace!(target: "messaging-tron", "Processing Response message with {} responses", resp.len());
				for res in resp {
					let commitment = hash_response::<Hasher>(res);
					let request_commitment = hash_request::<Hasher>(&res.request());
					log::trace!(
						target: "messaging-tron", "Response commitment: {:?}, request commitment: {:?}",
						commitment,
						request_commitment
					);
					if receipts.contains(&commitment) {
						log::trace!(target: "messaging-tron", "Adding receipt for response commitment {:?}", commitment);
			...
			_ => {
				log::trace!(target: "messaging-tron", "Skipping non-request/response message");
			},
		}
	}

	log::info!(target: "messaging-tron", "handle_message_submission complete: {} receipts", results.len());
...
pub async fn submit_messages(
	...
	log::trace!(target: "messaging-tron", "submit_messages called with {} messages", messages.len());

	log::trace!(target: "messaging-tron", "Calling generate_contract_calls");
	let (calls, _gas_price) = generate_contract_calls(&client.evm, &messages, false).await?;
	log::trace!(target: "messaging-tron", "generate_contract_calls returned {} calls", calls.len());

	let mut events = BTreeSet::new();

	for (index, call) in calls.iter().enumerate() {
		log::trace!(target: "messaging-tron", "Processing call {} of {}", index + 1, calls.len());

		let calldata = match call.input.input() {
			Some(bytes) => {
				log::trace!(target: "messaging-tron", "Call {} has calldata of {} bytes", index, bytes.len());
				bytes.clone()
			},
			None => {
				log::error!(target: "messaging-tron", "Message at index {index} produced empty calldata");
		...
		log::trace!(
			target: "messaging-tron", "Call {} calldata (hex): {}...",
			index,
		...
		log::info!(
			target: "messaging-tron", "Submitting {label} to {:?} ({} bytes calldata) ...",
		...
		match trigger_sign_broadcast(client, &calldata_hex).await {
			Ok(info) => {
				log::trace!(target: "messaging-tron", "trigger_sign_broadcast returned for call {}", index);

				if info.succeeded() {
					let energy = info.receipt.as_ref().map(|r| r.energy_usage_total).unwrap_or(0);

					log::info!(
						target: "messaging-tron", "{label} succeeded (tx={}, block={}, energy={energy})",
						info.id,
						info.block_number,
					);
					...
					log::trace!(
						target: "messaging-tron", "Extracted {} commitment hashes from tx {}",
						msg_events.len(),
						info.id
					);
					...
					log::error!(target: "messaging-tron", "{label} reverted (tx={}): {reason}", info.id);
					return Err(anyhow!("Transaction reverted: {}", reason));
				}
			},
			Err(err) => {
				log::error!(target: "messaging-tron", "{label} submission failed: {err:#}");
				return Err(err);
			},
		}
	}
	...
	if !events.is_empty() {
		log::trace!(
			target: "messaging-tron", "Got {} receipts from executing on {:?}",
			events.len(),
			client.evm.state_machine,
		);
	}

	log::trace!(target: "messaging-tron", "submit_messages complete: {} events", events.len());
	Ok(events)
}
...
async fn trigger_sign_broadcast(
	...
	log::trace!(target: "messaging-tron", "trigger_sign_broadcast: preparing request");

	// Query handler address dynamically
	let handler_address = client.handler_address().await?;

	log::trace!(
		target: "messaging-tron", "owner_address: {}, contract_address: {}, fee_limit: {}",
	...
	log::trace!(target: "messaging-tron", "Estimated energy: {}", estimated_energy);
	...
	log::trace!(target: "messaging-tron", "Calling POST /wallet/triggersmartcontract");
	...
	log::trace!(target: "messaging-tron", "triggerSmartContract response received, checking result");
	...
	log::trace!(target: "messaging-tron", "Got unsigned transaction with tx_id={}", tx_id);

	log::trace!(target: "messaging-tron", "Signing transaction");
	let signed = SignedTransaction::sign(unsigned, &client.secret_key)
		.context("failed to sign TRON transaction")?;

	log::trace!(target: "messaging-tron", "Broadcasting transaction tx_id={}", tx_id);
	...
	log::trace!(target: "messaging-tron", "Broadcast response received, checking result");
	broadcast_result.into_result().context("broadcastTransaction rejected")?;

	log::trace!(target: "messaging-tron", "Broadcast tx_id={tx_id}");

	log::trace!(target: "messaging-tron", "Waiting for receipt for tx_id={}", tx_id);
...
pub async fn wait_for_receipt(api: &TronApi, tx_id: &str) -> anyhow::Result<TransactionInfo> {
	log::trace!(target: "messaging-tron", "wait_for_receipt: starting poll for tx_id={}", tx_id);
	...
	loop {
		...
		if elapsed >= max_duration {
			log::error!(
				target: "messaging-tron", "wait_for_receipt: timeout after {} attempts ({} seconds) for tx: {}",
				attempt,
				elapsed.as_secs(),
				tx_id
			);
		...
		log::trace!(target: "messaging-tron", "wait_for_receipt: attempt {} for tx_id={}", attempt, tx_id);

		match api.get_transaction_info(tx_id).await {
			Ok(Some(info)) => {
				log::trace!(
					target: "messaging-tron", "wait_for_receipt: receipt found after {} attempts ({} seconds) for tx: {}",
					attempt,
					elapsed.as_secs(),
					tx_id
				);
				log::trace!(target: "messaging-tron", "Receipt found for tx: {tx_id}");
				return Ok(info);
			},
			Ok(None) => {
				log::trace!(
					target: "messaging-tron", "Receipt not yet available for {tx_id}, retrying in {}s (attempt {})",
			...
			Err(err) => {
				log::warn!(
					target: "messaging-tron", "Error querying receipt for {tx_id} (attempt {}): {err:#}, will retry",
					attempt
				);
			},
		}
...
fn extract_commitment_hashes(info: &TransactionInfo) -> BTreeSet<H256> {
	log::trace!(
		target: "messaging-tron", "extract_commitment_hashes: processing {} log entries for tx {}",
		info.log.len(),
		info.id
	);
	...
	log::trace!(target: "messaging-tron", "Looking for request_topic: {:?}", request_topic);
	log::trace!(target: "messaging-tron", "Looking for response_topic: {:?}", response_topic);

	let mut hashes = BTreeSet::new();

	for (idx, log_entry) in info.log.iter().enumerate() {
		log::trace!(target: "messaging-tron", "Processing log entry {} with {} topics", idx, log_entry.topics.len());

		if log_entry.topics.is_empty() {
			log::trace!(target: "messaging-tron", "Log entry {} has no topics, skipping", idx);
			continue;
		}
		...
		let topic0 = match hex::decode(&log_entry.topics[0]) {
			Ok(bytes) if bytes.len() == 32 => {
				...
				log::trace!(target: "messaging-tron", "Log entry {} topic0: {:?}", idx, h);
				h
			},
			Ok(bytes) => {
				log::trace!(target: "messaging-tron", "Log entry {} topic0 has wrong length: {}", idx, bytes.len());
				continue;
			},
			Err(e) => {
				log::trace!(target: "messaging-tron", "Log entry {} topic0 hex decode failed: {}", idx, e);
				continue;
			},
		};

		if topic0 != request_topic && topic0 != response_topic {
			log::trace!(target: "messaging-tron", "Log entry {} topic0 doesn't match request/response topics", idx);
			continue;
		}

		log::trace!(
			target: "messaging-tron", "Log entry {} matches {} event",
			idx,
		...
		if log_entry.topics.len() >= 2 {
			if let Ok(bytes) = hex::decode(&log_entry.topics[1]) {
				if bytes.len() == 32 {
					...
					log::trace!(target: "messaging-tron", "Extracted commitment: {:?}", commitment);
					hashes.insert(commitment);
				} else {
					log::trace!(target: "messaging-tron", "Log entry {} topic1 has wrong length: {}", idx, bytes.len());
				}
			} else {
				log::trace!(target: "messaging-tron", "Log entry {} topic1 hex decode failed", idx);
			}
		} else {
			log::trace!(target: "messaging-tron", "Log entry {} doesn't have topic1", idx);
		}
	}

	log::trace!(target: "messaging-tron", "extract_commitment_hashes: extracted {} commitments", hashes.len());
	hashes
}
...
fn decode_revert_message(info: &TransactionInfo) -> String {
	log::trace!(target: "messaging-tron", "decode_revert_message for tx {}", info.id);
	log::trace!(target: "messaging-tron", "res_message: {:?}", info.res_message);
	log::trace!(target: "messaging-tron", "result: {:?}", info.result);

	let decoded = info
		.res_message
		.as_deref()
		.and_then(|hex_msg| {
			log::trace!(target: "messaging-tron", "Attempting to decode hex res_message: {}", hex_msg);
			hex::decode(hex_msg).ok()
		})
		.and_then(|bytes| {
			log::trace!(target: "messaging-tron", "Attempting UTF-8 decode of {} bytes", bytes.len());
			String::from_utf8(bytes).ok()
		})
		.unwrap_or_else(|| {
			let fallback = info.result.clone().unwrap_or_else(|| "unknown error".into());
			log::trace!(target: "messaging-tron", "Using fallback revert message: {}", fallback);
			fallback
		});

	log::trace!(target: "messaging-tron", "Decoded revert message: {}", decoded);
	decoded
}
...
async fn post_json<Req: Serialize, Res: serde::de::DeserializeOwned>(
	...
	log::trace!(target: "messaging-tron", "post_json: POST {}", url);

	let client = reqwest::Client::new();
	let mut builder = client.post(&url).json(body);

	if let Some(key) = api_key {
		log::trace!(target: "messaging-tron", "post_json: Adding TRON-PRO-API-KEY header");
		builder = builder.header("TRON-PRO-API-KEY", key);
	}

	log::trace!(target: "messaging-tron", "post_json: Sending request to {}", url);
	let resp = builder.send().await.with_context(|| format!("POST {url} failed"))?;

	let status = resp.status();
	log::trace!(target: "messaging-tron", "post_json: Received HTTP {} from {}", status, url);

	if !status.is_success() {
		let text = resp.text().await.unwrap_or_default();
		log::error!(target: "messaging-tron", "post_json: HTTP error {}: {}", status, text);
		return Err(anyhow!("POST {url} returned HTTP {status}: {text}"));
	}

	let text = resp.text().await.context("failed to read response body")?;
	log::trace!(target: "messaging-tron", "post_json: Response body length: {} bytes", text.len());
...
async fn estimate_transaction_energy(
	...
	log::trace!(target: "messaging-tron", "Estimating transaction energy");
	...
	log::trace!(
		target: "messaging-tron", "Raw energy estimate: {}, with 20% margin: {}",
		estimated_energy,
		energy_with_margin
	);
...
async fn purchase_energy_via_catfee(
	...
	log::info!(target: "messaging-tron", "Purchasing {} energy units via CatFee", energy_required);
	...
	log::trace!(target: "messaging-tron", "Using receiver address (base58): {}", receiver_base58);
	...
	log::info!(target: "messaging-tron", "Energy purchase completed successfully");
