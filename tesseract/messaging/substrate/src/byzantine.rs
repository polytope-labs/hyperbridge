	...
	H256: From<HashFor<C>>,
{
	async fn check_for_byzantine_attack(
		...
		let Some(block_hash) =
			...
			log::info!(
				target: "messaging-substrate", "Vetoing state commitment for {} on {}: block header not found for {}",
		...
		if finalized_state_commitment.state_root != state_root.into() {
			log::info!(
				target: "messaging-substrate", "Vetoing state commitment for {} on {}, state commitment mismatch",
				self.state_machine_id().state_id,
				counterparty.state_machine_id().state_id
			);
			counterparty.veto_state_commitment(height).await?;
