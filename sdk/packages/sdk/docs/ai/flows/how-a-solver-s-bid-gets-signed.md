...
V3 bids use `fillOrder(order, options)` with signed takes in `options.inputs`; empty inputs select ordinary settlement. Before signing any v3 bid, the SDK checks that `fillOrderSelector()` equals `0x68ddf058` on the destination gateway, configured SolverAccount implementation, and live solver delegation. Missing or mismatched support refuses the bid, including ordinary and phantom bids. Input amounts are maxima, and `previewRateFill` reports their integer-rounded releases. Historical v1/v2 codecs remain available for older gateways; an upgraded gateway requires new calldata and a compatible delegated account.
