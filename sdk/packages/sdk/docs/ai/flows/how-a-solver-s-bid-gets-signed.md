...
On a release-3 gateway the bid carries one take per leg in `options.inputs`, paired with its output. Before signing, the gateway, the configured `SolverAccount` implementation and the solver's live delegation must all report release 3, for ordinary and phantom bids alike. `previewRateFill` shows what a take actually releases after rounding.
