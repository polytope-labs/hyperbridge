---
title: Intent Gateway Filler
description: The Hyperbridge Intent Filler is an automated system that monitors cross-chain orders and fills them profitably. It acts as a bridge between users who want to move assets across chains and the underlying Hyperbridge protocol, earning fees by providing liquidity and execution services.
---
...
The confirmation policy is a **critical security feature** that determines how many blockchain confirmations to wait for before processing a cross-chain order. This prevents attacks from blockchain reorganizations (reorgs) where transactions can be reversed.
...
Confirmation policies are configured **per strategy**. BasicFiller uses the global `[confirmationPolicies]` section directly. Advanced strategies like HyperFX can also define their own confirmation policies; cross-chain orders without an applicable policy are skipped rather than being processed with zero confirmations.
