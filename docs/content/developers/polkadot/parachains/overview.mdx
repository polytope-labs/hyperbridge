...
|------|--------|-------------|
| `add_parachain` | RootOrigin | This call allows a priviledged origin to add a new parachain to the list of connected parachains. When a new parachain is added, the inherent provider will add state proofs of the parachain's latest header into subsequent consensus messages. The arguments are: 1. **id**: The parachain id of the sibling parachain. 2. **slotDuration**: The slot duration in **milliseconds** for the sibling parachain. |
| `update_parachain_consensus` | Inherent | This is an inherent call that is used by the collator to include the consensus proofs for newly finalized parachain headers. |
