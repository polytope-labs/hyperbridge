...
Retrieves the state commitment at a specific height.
...
**Note:** IHost extends IDispatcher and inherits all dispatch-related events. See [IDispatcher Events](/developers/evm/api/idispatcher#events) for `PostRequestEvent`, `GetRequestEvent`, and `RequestFunded`.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `amount` | `uint256` | Amount that was withdrawn |
| `beneficiary` | `address` | The beneficiary address for this withdrawal |
| `token` | `address` | The token that was withdrawn (`address(0)` for native token) |
