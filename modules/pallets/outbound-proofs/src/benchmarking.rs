...
// `beefy_next_authority_set.id`), which would make the resulting update
// non-mandatory. We rewrite `next_authorities.id` to 0x1275 so the verifier's
// authority-set rotation path runs and the pallet observes
// `new_validator_set_id > old_validator_set_id` — the mandatory branch, which
// avoids a parachain-id match against `HostStateMachine`. The SP1 public inputs
// only commit to `authority.keyset_commitment` and `authority.len`, not `id`,
// so the proof still verifies.
