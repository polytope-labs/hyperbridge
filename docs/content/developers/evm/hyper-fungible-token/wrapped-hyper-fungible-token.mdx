...
uint256 nativeFee = IHyperFungibleToken(address(wrapper)).quoteNative(params);
...
uint256 feeTokenAmount = IHyperFungibleToken(address(wrapper)).quote(params);
address feeToken = IDispatcher(host()).feeToken();
// Approve the fee token transfer from the caller
IERC20(feeToken).approve(address(wrapper), feeTokenAmount);
// Dispatch the transfer, fees will be withdrawn from the caller
IHyperFungibleToken(address(wrapper)).send(params);
```
...
IHyperFungibleToken(address(wrapper)).send{value: amount + nativeFee}(params);
```
