...
// Wait for TRE to mine a block so the previous transaction is confirmed
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const BLOCK_TIME = 6000; // 6 seconds — TRE default block interval
...
module.exports = async function (deployer, network, accounts) {
    ...
    const hyperbridgeStr = isMainnet ? `POLKADOT-${paraId}` : `KUSAMA-${paraId}`;
    ...
    // ═════════════════════════════════════════════════════════════════════
    console.log("→ Deploying EthereumTrieDB library ...");
    await deployer.deploy(EthereumTrieDB);
    console.log("  ✓ EthereumTrieDB:", EthereumTrieDB.address);

    console.log("→ Linking EthereumTrieDB → MerklePatricia ...");
    await deployer.link(EthereumTrieDB, MerklePatricia);

    console.log("→ Deploying MerklePatricia library ...");
    await deployer.deploy(MerklePatricia);
    console.log("  ✓ MerklePatricia:", MerklePatricia.address);

    console.log("→ Linking MerklePatricia → HandlerV1 ...");
    await deployer.link(MerklePatricia, HandlerV1);
    ...
    if (consensusState && consensusState.length > 2) {
        ...
        console.log("  ⚠ CONSENSUS_STATE not provided — skipping setConsensusState");
    }
