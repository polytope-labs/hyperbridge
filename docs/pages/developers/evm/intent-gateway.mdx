...
> **Looking to integrate?** Check out the [Intent Gateway SDK Guide](/developers/evm/intent-gateway-sdk) for a complete TypeScript implementation with automated fee calculation, order placement, and cancellation handling.
...
**Important**: To calculate the correct profitable fees for your orders, see the [Intent Gateway SDK Guide](./intent-gateway-sdk.mdx#estimating-profitable-fees) which provides automated fee estimation.
...
**Note**: The fee amount shown above (2.8 DAI) is for illustration. For production use, always calculate fees programmatically using the [Intent Gateway SDK](./intent-gateway-sdk.mdx#estimating-profitable-fees) to ensure fillers find your order profitable.
...
**Note**: Order cancellation involves complex proof generation and submission. For a complete guide with working examples, see the [Order Cancellation section in the SDK Guide](./intent-gateway-sdk.mdx#canceling-orders).
...
**Important**: This is an illustrative example. Actual fees vary based on gas prices, chain congestion, and token liquidity. Use the [Intent Gateway SDK's `estimateFillOrder()` method](./intent-gateway-sdk.mdx#estimating-profitable-fees) to calculate accurate fees for your specific order.
...
1. **Set Appropriate Fees**: Use the [Intent Gateway SDK](./intent-gateway-sdk.mdx#estimating-profitable-fees) to calculate profitable fees automatically - higher fees attract fillers faster
2. **Reasonable Deadlines**: Allow sufficient time for filling (30-60 minutes recommended)
3. **Monitor Orders**: Track order status via the SDK's `isOrderFilled()` method or query client
4. **Handle Failures**: Implement cancellation logic for expired orders using the [SDK's cancellation feature](./intent-gateway-sdk.mdx#canceling-orders)
...
- **[Intent Gateway SDK Guide](./intent-gateway-sdk.mdx)** - Complete guide on using the TypeScript SDK for placing orders with correct fees and handling cancellations
