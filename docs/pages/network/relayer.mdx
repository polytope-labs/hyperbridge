...
The tesseract relayer command line interface expects only a single argument, which is the path to it's configuration file. Because it resides in a docker container, you will need to [map the directory](https://docs.docker.com/storage/bind-mounts/#start-a-container-with-a-bind-mount) where you've stored the configuration file on your host into the docker conatiner.


```bash
docker run -v /dir/to/config/on/host:/home/root polytopelabs/tesseract:latest --config=/home/root/config.toml
