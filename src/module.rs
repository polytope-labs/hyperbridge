...
pub trait IISMPModule {
    ...
    /// Called by the router on a module, to notify module of a response to a previously sent out request
    fn on_response(response: Response) -> Result<(), Error>;
    /// Called by the router on a module, to notify module of requests that were previously sent but have now timed-out
