...
2. `buildSimplexPaymasterData` picks the first configured stablecoin (USDC, then USDT — `configuredFeeTokens`) with a balance of at least one whole token (`selectToken`). When none qualifies it returns the balances it read instead of paymaster data, and step 1 records them as `simplex: solver USDC balance 0 < 1000000, USDT balance 0 < 1000000`, or `simplex: no fee token configured`. Otherwise:
   - `permit2 = configService.getPermit2Address(chain)` must be configured and `paymasterSupportsPermit2` (a cached `PERMIT2()` read on the paymaster) must hold, else the builder throws an error naming the reason; `buildPaymasterAndData` catches it, warns, and returns it as the `type: "none"` reason. Then:
     ...
     - Allowance short otherwise: `sendFundedApprove` — pre-check the native balance against the whole sequence, reset a stale non-zero allowance to zero first (USDT rule), then `approve(Permit2, max)` from the solver EOA, up to two txs, each waiting two confirmations. Then `buildPermit2Mode`.
