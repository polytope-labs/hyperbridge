...
use tesseract_substrate::{
	config::KeccakSubstrateChain,SubstrateClient, SubstrateConfig,
};
...
async fn relay_get_response_message(
	...
	let hashed_key:[u8;16] = hex::decode("0x6acc85a7ed191beef75dc62a9eb8b353".strip_prefix("0x").unwrap()).unwrap().try_into().unwrap();
	let encoded_value = chain_a_client.client.storage().at_latest().await?.fetch(&api::storage().ismp_demo().get_responses(hashed_key)).await?.unwrap();
...
async fn get_request_works() -> Result<(), anyhow::Error> {
	...
	let value_returned_encoded =
		relay_get_response_message(chain_a_sub_client, chain_b_sub_client.clone(), get_request_event)
			.await?;
