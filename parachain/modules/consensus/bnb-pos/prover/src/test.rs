...
use bnb_pos_verifier::{primitives::Header, verify_bnb_header};
use tokio::time::interval;
use bnb_pos_verifier::primitives::{compute_epoch, EPOCH_LENGTH};
...
async fn verify_bnb_pos_headers() {
    ...
    let (_, validators_data) = prover.fetch_proofs_and_validators::<Host>(validator_change_header).await.unwrap();
    ...
    loop {
        ...
        if initial_header_epoch == latest_header_epoch {
            ...
            println!("Successfully verified header {:?} in epoch {:?}", latest_header.number.low_u64(), latest_header_epoch);
        }
        ...
        if initial_header_epoch > latest_header_epoch {
            for epoch in (initial_header_epoch.clone() + 1) .. latest_header_epoch {
                ...
                let last_header_in_epoch= prover.fetch_header(last_header_number_epoch.clone()).await.unwrap();

                let previous_epoch = last_header_in_epoch - EPOCH_LENGTH;
                let (previous_epoch_bnb_proof, _previous_epoch_validators_data) = prover.fetch_proofs_and_validators::<Host>(previous_epoch).await.unwrap();
                let previous_epoch_validator_set_size = previous_epoch_bnb_proof.validator_set_size;

                let validator_set_change_epoch: u64 =  previous_epoch_validator_set_size as u64/2 + previous_epoch;

                if epoch == validator_set_change_epoch {

                    let validator_set_change_epoch_header= prover.fetch_header(epoch.clone()).await.unwrap();
                    let (_, validator_set_change_validators_data) = prover.fetch_proofs_and_validators::<Host>(validator_set_change_epoch_header).await.unwrap();
                ...
                println!("Successfully verified header {:?} in new epoch {:?}", latest_header.number.low_u64(), last_header_number_epoch);
            }
        }
    }
}
