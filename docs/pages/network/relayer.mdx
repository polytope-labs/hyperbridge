...
<br />
In the fee accumulation phase, the relayer submits state proofs of messages all delivered to hyperbridge using the `accumulate_fees` extrinsic on the `RelayerFees` pallet. If the proof verification is successful, the total amount of unclaimed fees for the relayer will be recorded.
<br />
...
```bash
    docker run -v /dir/to/config_and_database/on/host:/home/root polytopelabs/tesseract:latest --config=/home/root/config.toml --db=/home/root/dev.db accumulate-fees
```
...
```bash
    docker run -v /dir/to/config_and_database/on/host:/home/root polytopelabs/tesseract:latest --config=/home/root/config.toml --db=/home/root/dev.db accumulate-fees --withdraw
```
