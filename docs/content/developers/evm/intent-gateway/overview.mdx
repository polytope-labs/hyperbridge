...
Before signing, the SDK requires release `3` from `version()` on the gateway, configured account
implementation, and live delegation. Gateway release 3 uses FillOptions ABI version 3. The account protects the new and both historical selectors against
