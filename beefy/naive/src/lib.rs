			...
			loop {
				...
				let signed_commitment =
					match result {
						...
						Err(err) => {
							log::error!("zkBeefy prover encountered an error fetching justification: {err:?}");
							continue;
						},
					};
				...
				// send, if no one is listening, abort
				if clone_client.sender.send((message, latest_height, set_id)).is_err() {
	...
	pub async fn next_epoch_justification(&self, start: u64) -> anyhow::Result<Option<R::Hash>> {
		for i in start..=(start + 50) {
			let hash = if let Some(hash) =
				self.prover.inner().relay.rpc().block_hash(Some(i.into())).await?
			{
				hash
			} else {
				continue;
	...
	pub async fn sync(
		...
		if !(current_set_id..=(current_set_id + 1)).contains(&latest_set_id) {
			...
			if let Some((block_hash, _)) = block_hash_and_set_id.iter().next() {
				...
				let set_id = signed_commitment.commitment.validator_set_id;
				let consensus_proof =
					self.prover.consensus_proof(signed_commitment, consensus_state.clone()).await?;
