...
async fn beefy_consensus_updates() -> anyhow::Result<()> {
    ...
    let task = tokio::spawn({
        ...
        async move {
            tesseract_consensus::relay(chain_a, chain_b, Default::default())
                .await
                .unwrap()
        }
    });
...
async fn sync_committee_consensus_updates() -> anyhow::Result<()> {
    ...
    let chain_b = {
        ...
        let sync_commitee_config = SyncHostConfig {
            beacon_http_urls: vec![beacon_url],
...
async fn l2_state_machine_notification() -> anyhow::Result<()> {
    ...
    let state_id = StateMachineId {
        state_id: StateMachine::Kusama(para_id),
        consensus_state_id: *b"PARA",
    };

    let mut stream_base = base
        .state_machine_update_notification(state_id.clone())
        .await?;
    let mut stream_op = op
        .state_machine_update_notification(state_id.clone())
        .await?;
    let mut stream_arb = arb
        .state_machine_update_notification(state_id.clone())
        .await?;
