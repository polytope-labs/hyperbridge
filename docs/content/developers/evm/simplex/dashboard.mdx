...
<Callout type="warn">
    Loopback is machine-local, not user-local. On a shared machine, another OS user can reach the
    unauthenticated TCP dashboard. On macOS or Linux, use `--ui-socket` in an owner-only directory
    and forward that socket over SSH, or use the authenticated remote-access tunnel. Simplex Desktop
    already uses a private socket and opens no TCP listener. A Windows named pipe has a weaker
    operating-system ACL guarantee; do not treat it as an owner-only boundary from administrators.
</Callout>
...
| `--ui-socket <path>` | off | Serve the dashboard on a Unix socket instead of TCP. The socket is created mode `0600`; use it with SSH forwarding on a shared macOS or Linux host. On Windows this selects a named pipe, whose default ACL is weaker. |
...
simplex run --ui-socket /run/user/1000/simplex.sock
...
### Shared hosts: forward a private Unix socket

On a multi-user macOS or Linux host, avoid the TCP listener entirely. Put the socket in a directory
only the operator can traverse, then forward a local TCP port to that remote socket:

```bash lineNumbers
# On the solver host
install -d -m 700 "$HOME/.simplex-runtime"
simplex run --ui-socket "$HOME/.simplex-runtime/ui.sock"

# On the trusted workstation
ssh -N -L 8686:/home/operator/.simplex-runtime/ui.sock operator@your-server
```

Replace `/home/operator` with the absolute home directory on the solver host. Open
`http://localhost:8686` on the trusted workstation. The browser talks only to the local end of the
SSH forward; the shared host exposes no TCP dashboard, and the remote socket is mode `0600`.
