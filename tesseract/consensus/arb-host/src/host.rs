...
use ismp::messaging::{ConsensusMessage, CreateConsensusState, Message, StateCommitmentHeight};
use ismp_arbitrum::{ARBITRUM_CONSENSUS_CLIENT_ID, ArbitrumConsensusProof, ArbitrumConsensusType, ArbitrumUpdate, ConsensusState};
...
impl IsmpHost for ArbHost {
	async fn start_consensus(
		...
		let mut stream = counterparty.state_machine_update_notification(l1_state_machine_id.clone()).await?;
		let mut latest_height =
			counterparty.query_latest_height(l1_state_machine_id).await? as u64;
		while let Some(res) = stream.next().await {
			match res {
				Ok(event) => {
					match consensus_state.arbitrum_consensus_type {
						ArbitrumConsensusType::ArbitrumOrbit => {
							...
							if let Some(event) = latest_event {
								...
								let update = ArbitrumUpdate {
									state_machine_id: StateMachineId { state_id: self.state_machine, consensus_state_id: self.consensus_state_id },
