...
Currently, GET request fees are calculated with a minimum fee and context-based component, **In the future this may change**. The per-byte fee is queried from Hyperbridge (the host), and the actual fee is the maximum of either the minimum fee (32 bytes worth) or the context-based fee:
...
<Callout type="info">
For testing purposes on testnet, you can use the testnet fee token, [Hyper USD](https://sepolia.etherscan.io/address/0xA801da100bF16D07F668F4A49E1f71fc54D05177). The contract address is the same across several EVM chains. Check out the [guide](/developers/guides/testnet-fee-token) on how to get the token. 
</Callout>
...
Like [POST request timeouts](/developers/evm/dispatching/post-requests#timeouts), GET request timeouts require a cryptographic proof from the destination chain showing the timeout period has elapsed. Anyone can submit the timeout proof by calling [`IHandler.handleGetRequestTimeouts()`](/developers/evm/api/ihandler#handlegettimeouttimeouts). Unlike POST requests, GET requests don't have relayer fees, so there are no refunds to process on timeout.

The timeout is specified in seconds when dispatching the GET request:

```solidity lineNumbers title="Dispatching with timeout"
DispatchGet memory get = DispatchGet({
    dest: StateMachine.evm(137),  // Polygon
    keys: keys,
    height: 0,
...
│  (timeout period passed)│      │   to IHandler           │      │  (cleanup & rollback)   │
└─────────────────────────┘      └─────────────────────────┘      └─────────────────────────┘
```

To process a timed-out GET request, you need to construct a [`GetTimeoutMessage`](/developers/evm/api/ihandler#gettimeoutmessage) struct containing the cryptographic proof and the timed-out requests, then call the handler:

```solidity lineNumbers title="Processing a timeout"
// Build the GetTimeoutMessage struct with timed-out requests and proof
GetTimeoutMessage memory message = GetTimeoutMessage({
    timeouts: timedOutRequests,  // Array of GetRequest[]
    height: height,              // StateMachineHeight where timeout proof was generated
    proof: proofs                // Non-membership proofs showing requests weren't processed
});
...
<Callout type="info">
**GET requests do not have relayer fees**, so there are no fees to refund on timeout. The [`onGetTimeout()`](/developers/evm/api/iapp#ongettimeout) callback is primarily for cleanup and state management in your contract.
</Callout>

### Implementing the Timeout Callback

Your contract must implement [`onGetTimeout()`](/developers/evm/api/iapp#ongettimeout) to handle timed-out GET requests:
...
The [`onGetTimeout()`](/developers/evm/api/iapp#ongettimeout) callback can only be called by the [Host contract](/developers/evm/api/ihost). Always use the `onlyHost` modifier to protect this function.
</Callout>
