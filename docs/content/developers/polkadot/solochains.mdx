...
import addStateMachine from "./images/add_state_machine.png";
import grnpInit from "./images/grnp_init.png";
...
<img src={addStateMachine.src} />

## Calls

| Call | Origin | Description |
|------|--------|-------------|
| `add_state_machines` | RootOrigin | This adds new state machines which produce GRANDPA consensus proofs to the pallet whitelist, enabling trustless cross-chain communication with the state machines in question. The arguments are: 1. **stateMachine**: The state machine identifier for the blockchain to be connected. 2. **slotDuration**: The slot duration in **milliseconds** for the blockchain. |
...
You will of course need to pair this with the [relayer](/developers/network/relayer) which actually relays cross-chain messages.
