...
- **Account Key (sr25519)**: This is your main account key. It holds your reputation and is used to sign transactions. This key is what you used as the `signer` field in the `[hyperbridge]`
...
This is the master key to your account. The public address is what you will use as the signer in the `[hyperbridge]` section of the relayer configuration.
...
**Action 3: Link your keys**: You must now tell the blockchain that your Hyperbridge account (`AccountId`) will use these new session keys for collating.
You do this by calling the `session.setKeys` extrinsic from your Hyperbridge account.

- Navigate to `Developer -> Extrinsics` in the Polkadot UI.
- Select your Hyperbridge account(the one with the reputation score).
...
- Select your Hyperbridge account.
     Note that this is your relayer's public key. It must be the same account that is configured as the `signer` in your relayer setup and is the account that has been accruing reputation.
