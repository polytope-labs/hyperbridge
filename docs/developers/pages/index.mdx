---
title: Hyperbridge
description: Hyperbridge Protocol Specification
---

# Introduction

**Secure interoperability** requires the verification of consensus proofs, consensus fault proofs, state proofs and state transition validity proofs. Blockchains that want to securely communicate with each other must **perform these operations onchain**, so that they can be convinced of the finalized (irreversible) state of their counterparty.

These operations are very expensive and do not scale when we especially want to facilitate the communication of multiple blockchains. This neccessitates the coprocessor model where computations must be performed offchain and reported back onchain with cryptographic proofs of the correct execution.

## Hyperbridge

Hyperbrige is an instance of such a coprocessor. More formally, it is a [crypto-economic coprocessor](https://blog.polytope.technology/cryptoeconomic-coprocessors). These types of coprocessors are simply put, blockchains that leverage their [consensus proofs](https://research.polytope.technology/consensus-proofs) to attest to the correctness of the **computations performed onchain**. As a result, they are more decentralized, fault-tolerant, uncensorable and most importantly, unstoppable.

Hyperbridge is a hyper-scalable coprocessor for **cryptographically secure** interoperability. It achievs this by aggregrating the finalize states of all of it's connected chains unto it's chain. Using only a single consensus proof to any chain to prove the finality of the aggregated state. Hyperbridge is capable of scaling trust-free interoperability to all blockchains.

## Runtime Modules

These are the runtime modules that are included in the Hyperbridge blockchain. These modules are responsible for implementing the various components of the protocol, such as the consensus and state machine verifiers, cross-chain governance, relayer incentivization, and other components. The runtime modules included in Hyperbridge are:

- [ISMP](/modules/ismp)
- [ISMP Parachain](/modules/ismp-parachain)
- [ISMP Sync Committee](/modules/sync-committee)
- [Fishermen](/modules/fishermen)
- [Host Executive](/modules/host-executive)
- [Relayer](/modules/relayer)
