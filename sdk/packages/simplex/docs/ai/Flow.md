...
2. `buildSimplexPaymasterData` picks the first configured stablecoin (USDC, then USDT — `configuredFeeTokens`) with a balance of at least one whole token (`selectToken`). When none qualifies it returns the balances it read instead of paymaster data, and step 1 records them as `simplex: solver USDC balance 0 < 1000000, USDT balance 0 < 1000000`, or `simplex: no fee token configured`. Otherwise it authorizes in PERMIT2 mode, the only mode simplex sends:
   - `permit2 = configService.getPermit2Address(chain)` must be configured and `paymasterSupportsPermit2` (a cached `PERMIT2()` read on the paymaster) must hold, else the builder throws an error naming the reason; `buildPaymasterAndData` catches it, warns, and returns it as the `type: "none"` reason. Then:
     - Permit2 allowance below $5: bootstrap with `sendFundedApprove`: pre-check the native balance against the whole sequence, reset a stale non-zero allowance to zero first (USDT rule), then `approve(Permit2, max)` from the solver EOA, up to two txs, each waiting two confirmations. This is the one point where a solver still needs native, and it happens once per fee token per chain.
...
4. On chain (`evm/src/utils/SimplexPaymaster.sol`, unchanged): `_fetchDetails` accepts mode bytes `0x00` and `0x02` (`InvalidMode` otherwise, so a mode `0x01` payload with a standing allowance behind it is refused before any transfer) and, for mode 2, returns the permit deadline as `validUntil`; `_prefund` pulls mode 2 through `Permit2.permitTransferFrom` (owner = `userOp.sender`, to = paymaster, requestedAmount = oracle-derived prefund, capped by the signed amount); postOp refunds the unused part to the sender. Mode `0x00` (EIP-2612, executed by `_validatePaymasterUserOp` against the base `transferFrom` allowance) still exists on the permissionless contract — simplex simply never sends it.
...
- **`signTypedData` — the hot path.** Two callers:
    ...
    - `paymaster/permit2.ts` signs the per-op Permit2 `PermitTransferFrom` that lets the Simplex paymaster pull USDC/USDT for gas. It takes `Pick<Signer, "signTypedData">`, not the whole signer.
...
`DelegationService.setupDelegation(chain)` first asks `resolvePendingPermit2Approval` whether the chain's fee token has no Permit2 allowance yet. If so and the EOA's native balance covers one set-code tx (`nativeCoversDirectTx`), it goes straight to a direct type-0x04 transaction with `token.approve(Permit2, max)` batched in (650k gas floor, +60k for the approve): the sponsored path could not have used the paymaster before that approve landed anyway, so this is one native tx instead of a native approve followed by a sponsored op. If the batched tx reverts, `isDelegated` is checked before moving on: EIP-7702 applies authorization tuples even when execution reverts, so the delegation usually landed and only the approve is deferred to the first sponsored op.

Otherwise (nothing pending, or no native for the batched tx) it prefers the bundler: a no-op UserOp with the authorization attached, gas paid by the paymaster in stablecoins (`setupDelegationViaBundler`, signed with `signTypedData` through `UserOpSender`). The last fallback is a plain direct type-0x04 self-call, which needs native balance and fails with an explicit deficit log when the EOA cannot cover it.
