...
describe("automatic rate bid execution", () => {
	...
	it("executes full-escrow multi-leg quotes automatically", async () => {
		const multi = order()
		...
		const raw: FillerBid = { filler: "solver", userOp: op, deposit: 0n }
		const rate = bid(op)
		const fullQuote = { ...rate, inputs: multi.inputs, outputs: multi.output.assets }
		const executorCtx = {
			bundlerUrl: "x",
			intentsCoprocessor: { getBidsForOrder: vi.fn(async () => [raw]) },
			dest: {
		...
		vi.spyOn(manager, "buildBids").mockReturnValue([fullQuote])
		...
		expect((await stream.next()).value).toMatchObject({ status: "BID_SELECTED" })
		expect((await stream.next()).value).toMatchObject({ status: "FILLED" })
		expect(selectBest).toHaveBeenCalledWith(multi, [fullQuote], expect.any(Function), expect.any(Function))
		await stream.return()
	})
