...
- Local web UI: browser setup wizard (`simplex` with no config) that writes the config and starts the filler in-process, plus an operator page (status, pause/resume, graceful stop, balances, inflight price-curve edits persisted to the config file, overfill self-halt reset)
