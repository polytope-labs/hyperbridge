...
Validators are essentially signing the latest mmr root. These signatures can be found in the [justifications](https://spec.polkadot.network/sect-finality#defn-grandpa-justification) portion of the relay chain block. With signatures over the latest mmr root, consensus clients must:
 ...
 - Verify the latest leaf in the mmr tree, so they can be aware of the latest block number and hash.
...
A workaround for would be to leverage a SNARK circuit to perform the verification of these signatures alongside their membership proofs in the `keyset_commitment`. This effectively to amortizes the cost of verification, making it constant no matter how large the validator set grows. We leverage the AztecProtocol/barretenberg framework by the aztec team, which is able to prove the verification secp256k1 signatures in less than 2s. This ensures fast proving times for generating the zkBEEFY cosnensus proof.
