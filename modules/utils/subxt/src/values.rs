...
use ismp::messaging::{ConsensusMessage, CreateConsensusState, FraudProofMessage, Message, Proof, RequestMessage, ResponseMessage, StateCommitmentHeight, TimeoutMessage};
...
pub fn create_consensus_state_to_value(data: &CreateConsensusState) -> Value<()> {
    let challenge_periods_value = Value::unnamed_composite(
        data.challenge_periods.iter().map(|(sm, period)| {
...
fn state_commitment_to_value(sc: &StateCommitment) -> Value<()> {
    let overlay_root_value = match sc.overlay_root {
        Some(root) => Value::variant("Some", Composite::unnamed(vec![Value::from_bytes(root.as_bytes().to_vec())])),
...
fn versioned_host_params_to_composite(
    params: &VersionedHostParams<u128>,
) -> Composite<()> {
    match params {
        VersionedHostParams::V1(p) => {
            let inner_struct_as_value = Value::named_composite(
                substrate_host_params_to_vec(p)
            );
...
fn substrate_host_params_to_vec(
    params: &SubstrateHostParams<u128>,
) -> Vec<(String, Value<()>)> {
    let per_byte_fees_value = Value::unnamed_composite(
        params.per_byte_fees.iter().map(|(sm, fee)| {
            Value::unnamed_composite(vec![
                state_machine_to_value(sm),
                Value::u128(*fee),
            ])
...
fn evm_host_param_to_composite(param: &EvmHostParam) -> Composite<()> {
    let per_byte_fees_value = Value::unnamed_composite(
        param.per_byte_fees.iter().map(per_byte_fee_to_value)
    );
    let state_machines_value = Value::unnamed_composite(
        param.state_machines.iter().map(|id| Value::u128((*id).into()))
    );
    ...
    Composite::named(vec![
        ...
        ("default_per_byte_fee".to_string(), Value::from_bytes(param.default_per_byte_fee.encode())),
        ("state_commitment_fee".to_string(), Value::from_bytes(param.state_commitment_fee.encode())),
...
pub fn withdrawal_proof_to_value(proof: &WithdrawalProof) -> Value<()> {
    let commitments_value = Value::unnamed_composite(
        proof.commitments.iter().map(key_to_value)
    );
...
fn key_to_value(key: &Key) -> Value<()> {
    match key {
        ...
        Key::Response { request_commitment, response_commitment } => {
            let composite = Composite::named(vec![
                ("request_commitment".to_string(), Value::from_bytes(request_commitment.as_bytes().to_vec())),
                ("response_commitment".to_string(), Value::from_bytes(response_commitment.as_bytes().to_vec())),
            ]);
...
pub fn get_request_ismp_demo_to_value(params: &GetRequestIsmpDemo) -> Value<()> {
    let keys_value = Value::unnamed_composite(
        params.keys.iter().map(|key| Value::from_bytes(key.clone()))
    );
...
pub fn account_vec_to_value<C: Config>(accounts: &Vec<C::AccountId>) -> Value<()> {
    Value::unnamed_composite(
        accounts.iter().map(|account| Value::from_bytes(account.encode()))
    )
}

pub fn evm_hosts_btreemap_to_value(
    evm_hosts: &BTreeMap<StateMachine, H160>,
) -> Value<()> {
