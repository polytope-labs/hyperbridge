...
impl TendermintClient for HeimdallClient {
	...
	async fn validators(&self, height: u64) -> Result<Vec<Validator>, ProverError> {
		...
		loop {
			log::debug!(target: "consensus-polygon", "Requesting validators page {} with page_size {} for height {}",
			...
			log::trace!(target: "consensus-polygon", "Received {} validators in page {}",
			...
			if page == 1 {
				...
				log::info!(target: "consensus-polygon", "Total validators expected: {} for height {}",
			...
			log::trace!(target: "consensus-polygon", "Added {} validators, total collected: {}/{}",
					   all_validators.len() - validators_before, all_validators.len(), expected_total);

			if all_validators.len() >= expected_total {
				if all_validators.len() > expected_total {
					log::warn!(target: "consensus-polygon", "Collected more validators than expected! Got {}, expected {}. Truncating to expected count.",
							  all_validators.len(), expected_total);
				} else {
					log::debug!(target: "consensus-polygon", "Successfully collected all {} validators for height {}",
							   expected_total, height);
				}
