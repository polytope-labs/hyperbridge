...
export interface PaymasterOptions {
	...
	/** When true, skips permit detection and uses approve mode (for delegation UserOps). */
...
/** Dollar amount to authorize (permit or approve). Safe upper bound — unused gas is refunded. */
...
/** Verification gas limit when using EIP-2612 permit (higher due to permit verification). */
export const VERIFICATION_GAS_LIMIT_PERMIT = 250_000n
/** Verification gas limit when using on-chain approve (lower, no permit overhead). */
...
/** Post-operation gas limit (shared across all paymaster modes). */
