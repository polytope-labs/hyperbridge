...
export class GetRequestStatus {
	...
	constructor(
		...
		this.logger = ctx.logger.withTag("[GetRequestStatus]")
	}
	...
	async addGetRequestFinalityEvents(request: GetRequestWithStatus): Promise<GetRequestWithStatus> {
		...
		try {
			...
			const finalized = await this.proofFinalizer.buildGetResponseFinalized(
				request,
		...
		while (true) {
			switch (status) {
				...
				case RequestStatus.HYPERBRIDGE_DELIVERED: {
					...
					yield await this.proofFinalizer.streamGetResponseFinalized(signal, request, 1, response)
