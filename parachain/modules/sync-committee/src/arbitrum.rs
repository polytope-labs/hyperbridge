use crate::{
    ...
    utils::{get_contract_storage_root, to_bytes_32},
};
...
#[derive(RlpEncodable, codec::Encode, codec::Decode, Debug)]
pub struct Header {
...
pub fn derive_key<H: IsmpHost>(key: u64, slot: u8) -> Vec<u8> {
    let bytes = ethabi::encode(&[Token::Uint(U256::from(key)), Token::Int(U256::from(slot))]);
    H::keccak256(&bytes).0.to_vec()
}
...
pub fn verify_arbitrum_payload<H: IsmpHost + Send + Sync>(
    ...
    let block_number = payload.arbitrum_header.number;
    let timestamp = payload.arbitrum_header.timestamp;
    let state_root = payload.arbitrum_header.state_root;

    let header_hash = payload.arbitrum_header.hash::<H>();
    ...
    let state_hash_key = derive_key::<H>(payload.node_number, NODES_SLOT);
    let proof_value =
        match get_value_from_proof::<H>(state_hash_key, storage_root, payload.storage_proof)? {
