...
| `fee` | Optional relayer fee for delivering the response back to the source chain. Set to zero if self-relaying the `GetResponse`. |
...
const hyperbridge = await SubstrateChain.connect({
  ...
  wsUrl: "wss://nexus.rpc.polytope.technology",
...
GET request fees are optional relayer fees that incentivize third-party relayers to deliver the GET response back to the source chain. If your application prefers to self-relay the `GetResponse`, you can set the fee to zero.
