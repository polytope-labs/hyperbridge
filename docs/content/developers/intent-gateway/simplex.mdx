...
Pricing supports pools paired with USDC, USDT, or WETH. For WETH-paired pools, the venue chains the pool price with a WETH→USDC quote to derive the USD price. When multiple positions exist for the same exotic token on a chain, the most-liquid pool's price is used.
...
--
Confirm the configured positions contain a pool that pairs the exotic token with USDC, USDT, or WETH. Pools with other quote tokens cannot derive a USD price. Check that the position's `tokenId` is valid and owned by the solver. Aerodrome venues do not support live pricing yet.
