...
use pallet_session;
use polkadot_sdk::frame_support::{assert_ok, traits::{fungibles::Mutate, OnInitialize}};
use polkadot_sdk::sp_core::sr25519::Pair;
use polkadot_sdk::sp_core::Pair as _;
use polkadot_sdk::*;
...
fn test_new_collators_are_selected_based_on_reputation() {
    new_test_ext().execute_with(|| {
        ...
        pallet_session::QueuedKeys::<Test>::put(vec![
            (ALICE, crate::runtime::SessionKeys { aura: Pair::from_seed(ALICE.as_ref()).public().into() }),
            (BOB, crate::runtime::SessionKeys { aura: Pair::from_seed(BOB.as_ref()).public().into() }),
        ]);
        ...
        assert_ne!(Assets::balance(ReputationAssetId::get(), &DAVE), 0); // Dave still has balances left until after the next session
    });
}
...
fn test_reuse_previous_collators_if_not_enough_candidates() {
    new_test_ext().execute_with(|| {
        ...
        let mut new_collators = Session::validators();
        new_collators.sort();
        assert_eq!(new_collators, vec![ALICE, CHARLIE]); // Alice is chosen because the account has more balances than Bob
    });
}
...
fn test_collator_set_does_not_change_if_no_new_candidates() {
    new_test_ext().execute_with(|| {
        ...
        let mut new_collators = Session::validators();
        new_collators.sort();
        assert_eq!(new_collators, vec![ALICE, BOB]); // still use existing collator set since there are no candidates set up
    });
}
