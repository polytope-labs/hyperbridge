...
Hyperbridge charges a non-refundable protocol fee per byte of the request/response body. This fee is withdrawn from the contract account calling the `IsmpDispatcher`. 
Importantly, the contract must first approve the `EvmHost` to withdraw the exact amount before calling the dispatch function.
...
### Mitigating Gas Spikes with Relayer Fee Adjustments:
