...
The primary goal of a **Consensus Relayer** is to provide **proof-of-liveness**. They act as decentralized notaries, observing the finalized state of one blockchain and submitting a verifiable cryptographic proof of that state to Hyperbridge or any destination chain.
...
A **Messaging Relayer** acts as a courier, responsible for delivering specific user-initiated requests and responses between chains. Their job begins only *after* a Consensus Relayer has provided the necessary proof-of-liveness for the block containing the user's message.
...
| --------------------- | ------------------------------------------------------ | ------------------------------------------------------ |
| **Primary Role** | Provide proof-of-liveness for an entire chain.         | Deliver specific user messages and data payloads.      |
