...
async fn test_legacy_relay_state_drain() -> Result<(), anyhow::Error> {
	...
	for i in 0..blocks {
		...
		if i % 100 == 0 {
			let legacy =
				count_storage(&local_client, "IsmpParachain", "RelayChainStateCommitments").await?;
			let current =
				count_storage(&local_client, "IsmpParachain", "CurrentRelayChainStateRoots")
					.await?;
			eprintln!("Block {i}: legacy={legacy}, current={current}");
	...
	let final_legacy =
		count_storage(&local_client, "IsmpParachain", "RelayChainStateCommitments").await?;
	let final_current =
		count_storage(&local_client, "IsmpParachain", "CurrentRelayChainStateRoots").await?;

	eprintln!("Final: legacy={final_legacy}, current={final_current}");

	assert_eq!(
		final_legacy, 0,
		"legacy map should be fully drained (found {final_legacy} remaining)"
	);

	assert!(final_current <= 256, "new map should be bounded at 256 (found {final_current})");

	eprintln!("Test passed: legacy drained, new map bounded at {final_current}.");
