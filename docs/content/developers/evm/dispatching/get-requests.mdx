...
Contracts can only read storage items at heights that have been processed by the Hyperbridge blockchain. You can fetch the latest height for the chain you're interested in using the `latestStateMachineHeight()` function provided by the Hyperbridge SDK.

## Fees

Understanding the fee model for GET requests is important. Unlike POST requests, GET requests **only have protocol fees** - there are no relayer fees since responses are automatically delivered by the Hyperbridge protocol.

### Fee Components

| Component | Description | Calculation | Refundable |
|-----------|-------------|-------------|------------|
| **Protocol Fee** | Non-refundable fee charged by Hyperbridge for processing the request | `perByteFee(dest) × (baseFee + context.length)` | ❌ No |

### Protocol Fees for GET Requests

GET request fees are calculated based on a base fee plus the size of your context:

```solidity
protocolFee = perByteFee(destination) × (BASE_GET_REQUEST_SIZE + context.length)
```

Where:
- `BASE_GET_REQUEST_SIZE` = Fixed base size for GET request processing (typically ~200 bytes)
- `context.length` = Size of your application-specific context data

**Key Points:**
- Protocol fees are **non-refundable** even if the request times out
- Fees are paid in the **native token** of the source chain
- Larger context increases fees linearly
- No relayer fees exist for GET requests

### Calculating GET Request Fees

You can calculate the exact fee before dispatching:

```solidity
// Query the per-byte fee for the destination chain
uint256 perByteFee = IIsmpHost(host).perByteFee(StateMachine.evm(1)); // Ethereum

// Calculate total fee
uint256 baseSize = 200; // Approximate base size
uint256 contextSize = context.length;
uint256 protocolFee = perByteFee * (baseSize + contextSize);
```

### Fee Optimization Tips

1. **Minimize context size** - Only include essential data in context
2. **Reuse context patterns** - Use compact encoding schemes
3. **Batch reads** - Request multiple storage keys in one GET request instead of multiple requests
4. **Consider empty context** - If you can track requests another way, use empty context
...
contract MyApp is HyperApp {
    function onGetTimeout(GetRequest memory request)
        ...
        onlyHost
    {
        // Decode context to understand which request timed out
        (address user, address token) = abi.decode(
            request.context,
            (address, address)
        );
        
        // Clean up any pending state
        // Maybe notify the user or retry
